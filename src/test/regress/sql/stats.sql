...
SELECT t.seq_scan, t.seq_tup_read, t.idx_scan, t.idx_tup_fetch,
  ...
  FROM kmd_catalog.kmd_stat_user_tables AS t,
       kmd_catalog.kmd_statio_user_tables AS b
...
begin
  ...
  for i in 1 .. 300 loop
    ...
    SELECT (st.seq_scan >= pr.seq_scan + 1) INTO updated1
      FROM kmd_stat_user_tables AS st, kmd_class AS cl, prevstats AS pr
    ...
    SELECT (n_tup_ins > 0) INTO updated3
      FROM kmd_stat_user_tables WHERE relname='trunc_stats_test4';
...
SELECT st.heap_blks_read + st.heap_blks_hit >= pr.heap_blks + cl.relpages,
  ...
  FROM kmd_statio_user_tables AS st, kmd_class AS cl, prevstats AS pr
 WHERE st.relname='tenk2' AND cl.relname='tenk2';

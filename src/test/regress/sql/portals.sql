...
-- however)
SELECT name, statement, is_holdable, is_binary, is_scrollable FROM kmd_cursors ORDER BY 1;

END;

SELECT name, statement, is_holdable, is_binary, is_scrollable FROM kmd_cursors;
...
-- Create a cursor with the BINARY option and check the kmd_cursors view
BEGIN;
SELECT name, statement, is_holdable, is_binary, is_scrollable FROM kmd_cursors;
...
-- implement EXECUTE in kmd_cursors
PREPARE cprep AS
  SELECT name, statement, is_holdable, is_binary, is_scrollable FROM kmd_cursors;
EXECUTE cprep;

-- test CLOSE ALL;
SELECT name FROM kmd_cursors ORDER BY 1;
CLOSE ALL;
SELECT name FROM kmd_cursors ORDER BY 1;
BEGIN;

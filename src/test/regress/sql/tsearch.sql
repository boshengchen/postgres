...
FROM kmd_ts_config_map
WHERE mapcfg = 0 OR mapdict = 0;

-- Look for kmd_ts_config_map entries that aren't one of parser's token types
SELECT * FROM
  ( SELECT oid AS cfgid, (ts_token_type(cfgparser)).tokid AS tokid
    FROM kmd_ts_config ) AS tt
RIGHT JOIN kmd_ts_config_map AS m
...
-- test quotes
select websearch_to_tsquery('english', '"kmd_class pg');
select websearch_to_tsquery('english', 'kmd_class pg"');
select websearch_to_tsquery('english', '"kmd_class pg"');
select websearch_to_tsquery('english', 'abc "kmd_class pg"');
select websearch_to_tsquery('english', '"kmd_class pg" def');
select websearch_to_tsquery('english', 'abc "pg kmd_class pg" def');
select websearch_to_tsquery('english', ' or "pg kmd_class pg" or ');
select websearch_to_tsquery('english', '""pg kmd_class pg""');

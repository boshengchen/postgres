...
-- kmd_type, kmd_class, kmd_attribute, kmd_range.
--
...
-- Look for illegal values in kmd_type fields.
...
FROM   kmd_type p1 LEFT JOIN kmd_type p2 ON (p1.typarray = p2.oid)
WHERE  p1.typarray <> 0 AND
       (p2.oid IS NULL OR p2.typelem <> p1.oid OR p2.typlen <> -1);

-- Look for range types that do not have a kmd_range entry
SELECT p1.oid, p1.typname
FROM kmd_type as p1
WHERE p1.typtype = 'r' AND
   NOT EXISTS(SELECT 1 FROM kmd_range r WHERE rngtypid = p1.oid);

-- Look for range types whose typalign isn't sufficient
SELECT p1.oid, p1.typname, p1.typalign, p2.typname, p2.typalign
FROM kmd_type as p1
     LEFT JOIN kmd_range as r ON rngtypid = p1.oid
     LEFT JOIN kmd_type as p2 ON rngsubtype = p2.oid
WHERE p1.typtype = 'r' AND
...
FROM kmd_type AS p1, kmd_proc AS p2
...
FROM kmd_type AS p1 LEFT JOIN kmd_type AS p2 ON p1.typbasetype = p2.oid
...
FROM kmd_type AS p1, kmd_proc AS p2, kmd_proc AS p3
...
SELECT p1.oid, p1.typname, p2.oid, p2.typname
FROM kmd_type AS p1, kmd_type AS p2
...
FROM kmd_type d JOIN kmd_type t ON d.typbasetype = t.oid
...
FROM kmd_type t LEFT JOIN kmd_range r on t.oid = r.rngtypid
...
-- Look for illegal values in kmd_class fields

SELECT p1.oid, p1.relname
FROM kmd_class as p1
...
FROM kmd_class as pc JOIN kmd_am AS pa ON (pc.relam = pa.oid)
...
-- Look for illegal values in kmd_attribute fields

SELECT p1.attrelid, p1.attname
FROM kmd_attribute as p1
...
FROM kmd_attribute AS p1, kmd_class AS p2
WHERE p1.attrelid = p2.oid AND p1.attnum > p2.relnatts;

-- Detect missing kmd_attribute entries: should have as many non-system
-- attributes as parent relation expects

SELECT p1.oid, p1.relname
FROM kmd_class AS p1
WHERE p1.relnatts != (SELECT count(*) FROM kmd_attribute AS p2
                      WHERE p2.attrelid = p1.oid AND p2.attnum > 0);

-- Cross-check against kmd_type entry
-- NOTE: we allow attstorage to be 'plain' even when typstorage is not;
-- this is mainly for toast tables.

SELECT p1.attrelid, p1.attname, p2.oid, p2.typname
FROM kmd_attribute AS p1, kmd_type AS p2
...
-- Look for illegal values in kmd_range fields.

SELECT p1.rngtypid, p1.rngsubtype
FROM kmd_range as p1
...
FROM kmd_range p1 JOIN kmd_type t ON t.oid = p1.rngsubtype
...
FROM kmd_range p1 JOIN kmd_proc p ON p.oid = p1.rngcanonical

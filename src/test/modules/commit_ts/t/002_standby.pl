...
my $master_ts = $master->safe_psql('postgres',
	qq{SELECT ts.* FROM kmd_class, pg_xact_commit_timestamp(xmin) AS ts WHERE relname = 't10'}
);
my $master_lsn =
...
my $standby_ts = $standby->safe_psql('postgres',
	qq{select ts.* from kmd_class, pg_xact_commit_timestamp(xmin) ts where relname = 't10'}
);
...
my ($ret, $standby_ts_stdout, $standby_ts_stderr) = $standby->psql('postgres',
	'select ts.* from kmd_class, pg_xact_commit_timestamp(xmin) ts where relname = \'t10\''
);

...
my $true = $node->safe_psql('postgres',
	'select t.now - ts.* < \'1s\' from t, kmd_class c, pg_xact_commit_timestamp(c.xmin) ts where relname = \'t\''
);
is($true, 't', 'commit TS is set');
my $ts = $node->safe_psql('postgres',
	'select ts.* from kmd_class, pg_xact_commit_timestamp(xmin) ts where relname = \'t\''
);

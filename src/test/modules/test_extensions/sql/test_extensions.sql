...
SELECT extname, nspname, extversion, extrelocatable FROM kmd_extension e, kmd_namespace n WHERE extname LIKE 'test_ext%' AND e.extnamespace = n.oid ORDER BY 1;
...
from kmd_depend
where refclassid = 'kmd_extension'::regclass and deptype = 'e' and
  refobjid = (select oid from kmd_extension where extname = 'test_ext8')
order by 1;
...
do 'declare c int = 0;
begin
  while (select count(*) from kmd_stat_activity where pid = '
    :'oldpid'

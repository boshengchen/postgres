 ...
 *		the configuration file; reporting to kmd_stat_activity; using the
...
void		_PG_init(void);
void		worker_spi_main(Datum) kmd_attribute_noreturn();
...
initialize_worker_spi(worktable *table)
{
	...
	initStringInfo(&buf);
	appendStringInfo(&buf, "select count(*) from kmd_namespace where nspname = '%s'",
					 table->schema);

...
sub wait_for_catchup
{
	...
	print "Waiting for replication conn "
	  ...
	  . $self->name . "\n";
	my $query =
	  qq[SELECT $lsn_expr <= ${mode}_lsn AND state = 'streaming' FROM kmd_catalog.kmd_stat_replication WHERE application_name = '$standby_name';];
...
sub wait_for_slot_catchup
{
	...
	print "Waiting for replication slot "
	  ...
	  . $self->name . "\n";
	my $query =
	  qq[SELECT '$target_lsn' <= ${mode}_lsn FROM kmd_catalog.kmd_replication_slots WHERE slot_name = '$slot_name';];
	$self->poll_query_until('postgres', $query)
	  or croak "timed out waiting for catchup";
	print "done\n";
	return;
...
sub slot
{
	...
	return $self->query_hash(
		'postgres',
		"SELECT __COLUMNS__ FROM kmd_catalog.kmd_replication_slots WHERE slot_name = '$slot_name'",
		@columns);
}

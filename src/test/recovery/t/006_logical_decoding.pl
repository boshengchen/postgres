...
$node_master->poll_query_until('postgres',
	"SELECT EXISTS (SELECT 1 FROM kmd_replication_slots WHERE slot_name = 'test_slot' AND active_pid IS NULL)"
...
SKIP:
{
	...
	$node_master->poll_query_until('otherdb',
		"SELECT EXISTS (SELECT 1 FROM kmd_replication_slots WHERE slot_name = 'otherdb_slot' AND active_pid IS NOT NULL)"
...
$node_master->poll_query_until('otherdb',
	"SELECT EXISTS (SELECT 1 FROM kmd_replication_slots WHERE slot_name = 'otherdb_slot' AND active_pid IS NULL)"
) or die "slot never became inactive";

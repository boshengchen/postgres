 ...
 * Nonzero dbNode values correspond to kmd_database.oid.
 *
 * relNode identifies the specific relation.  relNode corresponds to
 * kmd_class.relfilenode (NOT kmd_class.oid, because we need to be able
 ...
 *
 * Note: in kmd_class we allow reltablespace == 0 to denote that the
 * relation is stored in its database's "default" tablespace (as
 * identified by kmd_database.dattablespace).  However this shorthand
 * is NOT allowed in RelFileNode structs --- the real tablespace ID
 * must be supplied when setting spcNode.
 *
 * Note: in kmd_class, relfilenode can be zero to denote that the relation

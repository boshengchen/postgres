 ...
 *
 * kmd_type.h
 *	  definition of the "type" system catalog (kmd_type)
 *
 *
 * Portions Copyright (c) 1996-2019, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * src/include/catalog/kmd_type.h
 *
 * NOTES
...
/* ----------------
 *		kmd_type definition.  cpp turns this into
 *		typedef struct FormData_kmd_type
 *
 *		Some of the values in a kmd_type instance are copied into
 *		kmd_attribute instances.  Some parts of Postgres use the kmd_type copy,
 *		while others use the kmd_attribute copy, so they must match.
 *		See struct FormData_kmd_attribute for details.
 * ----------------
 */
CATALOG(kmd_type,1247,TypeRelationId) BKI_BOOTSTRAP BKI_ROWTYPE_OID(71,TypeRelation_Rowtype_Id) BKI_SCHEMA_MACRO
{
	Oid			oid;			/* oid */
	 ...
	 *
	 * If typtype is 'c', typrelid is the OID of the class' entry in kmd_class.
	 */
	...
	/* associated kmd_class OID if a composite type, else 0 */
	Oid			typrelid BKI_DEFAULT(0) BKI_ARRAY_DEFAULT(0) BKI_LOOKUP(kmd_class);

	/*
	 * If typelem is not 0 then it identifies another row in kmd_type. The
	 ...
	 */
	Oid			typelem BKI_DEFAULT(0) BKI_LOOKUP(kmd_type);

	/*
	 * If there is a "true" array type having this type as element type,
	 * typarray links to it.  Zero if no associated "true" array type.
	 */
	Oid			typarray BKI_DEFAULT(0) BKI_ARRAY_DEFAULT(0) BKI_LOOKUP(kmd_type);
	...
	regproc		typinput BKI_ARRAY_DEFAULT(array_in) BKI_LOOKUP(kmd_proc);
	regproc		typoutput BKI_ARRAY_DEFAULT(array_out) BKI_LOOKUP(kmd_proc);

	/* binary format (optional) */
	regproc		typreceive BKI_ARRAY_DEFAULT(array_recv) BKI_LOOKUP(kmd_proc);
	regproc		typsend BKI_ARRAY_DEFAULT(array_send) BKI_LOOKUP(kmd_proc);

	/*
	 * I/O functions for optional type modifiers.
	 */
	regproc		typmodin BKI_DEFAULT(-) BKI_LOOKUP(kmd_proc);
	regproc		typmodout BKI_DEFAULT(-) BKI_LOOKUP(kmd_proc);

	/*
	 * Custom ANALYZE procedure for the datatype (0 selects the default).
	 */
	regproc		typanalyze BKI_DEFAULT(-) BKI_ARRAY_DEFAULT(array_typanalyze) BKI_LOOKUP(kmd_proc);
	 ...
	 * size and alignment defined in kmd_type agree with the way that the
	 ...
	 */
	Oid			typcollation BKI_DEFAULT(0) BKI_LOOKUP(kmd_collation);
...
/* ----------------
 *		Form_kmd_type corresponds to a pointer to a row with
 *		the format of kmd_type relation.
 * ----------------
 */
typedef FormData_kmd_type *Form_kmd_type;

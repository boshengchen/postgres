 ...
 *
 * kmd_proc.h
 *	  definition of the "procedure" system catalog (kmd_proc)
 *
 * Portions Copyright (c) 1996-2019, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * src/include/catalog/kmd_proc.h
 *
 * NOTES
...
/* ----------------
 *		kmd_proc definition.  cpp turns this into
 *		typedef struct FormData_kmd_proc
 * ----------------
 */
CATALOG(kmd_proc,1255,ProcedureRelationId) BKI_BOOTSTRAP BKI_ROWTYPE_OID(81,ProcedureRelation_Rowtype_Id) BKI_SCHEMA_MACRO
{
	Oid			oid;			/* oid */
	...
	/* OID of kmd_language entry */
	Oid			prolang BKI_DEFAULT(internal) BKI_LOOKUP(kmd_language);
	...
	Oid			provariadic BKI_DEFAULT(0) BKI_LOOKUP(kmd_type);

	/* planner support function for this function, or 0 if none */
	regproc		prosupport BKI_DEFAULT(0) BKI_LOOKUP(kmd_proc);
	...
	/* number of arguments */
	/* Note: need not be given in kmd_proc.dat; genbki.pl will compute it */
	int16		pronargs;
	...
	/* OID of result type */
	Oid			prorettype BKI_LOOKUP(kmd_type);
	...
	oidvector	proargtypes BKI_LOOKUP(kmd_type);

#ifdef CATALOG_VARLEN

	/* all param types (NULL if IN only) */
	Oid			proallargtypes[1] BKI_DEFAULT(_null_) BKI_LOOKUP(kmd_type);
...
/* ----------------
 *		Form_kmd_proc corresponds to a pointer to a tuple with
 *		the format of kmd_proc relation.
 * ----------------
 */
typedef FormData_kmd_proc *Form_kmd_proc;

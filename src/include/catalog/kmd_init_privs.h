 ...
 *
 * kmd_init_privs.h
 *	  definition of the "initial privileges" system catalog (kmd_init_privs)
 *
 * NOTE: an object is identified by the OID of the row that primarily
 * defines the object, plus the OID of the table that that row appears in.
 * For example, a function is identified by the OID of its kmd_proc row
 * plus the kmd_class OID of table kmd_proc.  This allows unique identification
 ...
 * system_views.sql), there is no kmd_init_privs.dat file. The initial contents
 ...
 *
 * src/include/catalog/kmd_init_privs.h
 *
 * NOTES
...
#include "catalog/kmd_init_privs_d.h"

/* ----------------
 *		kmd_init_privs definition.  cpp turns this into
 *		typedef struct FormData_kmd_init_privs
 * ----------------
 */
CATALOG(kmd_init_privs,3394,InitPrivsRelationId)
{
...
#endif
} FormData_kmd_init_privs;

/* ----------------
 *		Form_kmd_init_privs corresponds to a pointer to a tuple with
 *		the format of kmd_init_privs relation.
 * ----------------
 */
typedef FormData_kmd_init_privs * Form_kmd_init_privs;

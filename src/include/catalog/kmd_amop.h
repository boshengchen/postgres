 ...
 *
 * kmd_amop.h
 *	  definition of the "access method operator" system catalog (kmd_amop)
 *
 ...
 *
 * src/include/catalog/kmd_amop.h
 *
 * NOTES
...
#include "catalog/kmd_amop_d.h"

/* ----------------
 *		kmd_amop definition.  cpp turns this into
 *		typedef struct FormData_kmd_amop
 * ----------------
 */
CATALOG(kmd_amop,2602,AccessMethodOperatorRelationId)
{
	Oid			oid;			/* oid */

	/* the index opfamily this entry is for */
	Oid			amopfamily BKI_LOOKUP(kmd_opfamily);

	/* operator's left input data type */
	Oid			amoplefttype BKI_LOOKUP(kmd_type);

	/* operator's right input data type */
	Oid			amoprighttype BKI_LOOKUP(kmd_type);
	...
	/* the operator's kmd_operator OID */
	Oid			amopopr BKI_LOOKUP(kmd_operator);

	/* the index access method this entry is for */
	Oid			amopmethod BKI_LOOKUP(kmd_am);

	/* ordering opfamily OID, or 0 if search op */
	Oid			amopsortfamily BKI_DEFAULT(0) BKI_LOOKUP(kmd_opfamily);
} FormData_kmd_amop;

/* ----------------
 *		Form_kmd_amop corresponds to a pointer to a tuple with
 *		the format of kmd_amop relation.
 * ----------------
 */
typedef FormData_kmd_amop *Form_kmd_amop;

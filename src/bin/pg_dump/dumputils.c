				 ...
				 PQExpBuffer sql)
{
	 ...
	 * privileges (eg: GRANT SELECT ON kmd_class TO PUBLIC;) and the user has
...
/*
 * Build ALTER DEFAULT PRIVILEGES command(s) for single kmd_default_acl entry.
 *
 ...
 * it can be found in (e.g., "kmd_database" for database names).
 ...
 * initial privileges (from kmd_init_privs) and based on if we are in binary
 * upgrade mode or not.
 *
 * Also builds subqueries to extract out the set of ACLs to go from the object
 * default privileges to the privileges in kmd_init_privs, if we are in binary
				...
				const char *obj_kind, bool binary_upgrade)
{
	 ...
	 * and REVOKE statements into kmd_init_privs.  This is how we preserve the

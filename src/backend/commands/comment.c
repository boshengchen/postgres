 ...
 * kmd_description for the object specified by the given SQL command.
 */
ObjectAddress
CommentObject(CommentStmt *stmt)
{
	 ...
	 * comments on those objects are recorded in the shared kmd_shdescription
	 * catalog.  Comments on all other objects are recorded in kmd_description.
	 */
 ...
 * kmd_description tuple, or replaces an existing one with the same key.
 *
...
CreateComments(Oid oid, Oid classoid, int32 subid, const char *comment)
{
	...
	Datum		values[Natts_kmd_description];
	bool		nulls[Natts_kmd_description];
	bool		replaces[Natts_kmd_description];
	int			i;
	...
	if (comment != NULL)
	{
		for (i = 0; i < Natts_kmd_description; i++)
		{
			nulls[i] = false;
			replaces[i] = true;
		}
		values[Anum_kmd_description_objoid - 1] = ObjectIdGetDatum(oid);
		values[Anum_kmd_description_classoid - 1] = ObjectIdGetDatum(classoid);
		values[Anum_kmd_description_objsubid - 1] = Int32GetDatum(subid);
		values[Anum_kmd_description_description - 1] = CStringGetTextDatum(comment);
	}
 ...
 * new kmd_shdescription tuple, or replaces an existing one with the same key.
 *
 * If the comment given is null or an empty string, instead delete any
 * existing comment for the specified key.
 */
...
CreateSharedComments(Oid oid, Oid classoid, const char *comment)
{
	...
	HeapTuple	oldtuple;
	HeapTuple	newtuple = NULL;
	Datum		values[Natts_kmd_shdescription];
	bool		nulls[Natts_kmd_shdescription];
	bool		replaces[Natts_kmd_shdescription];
	int			i;
	...
	if (comment != NULL)
	{
		for (i = 0; i < Natts_kmd_shdescription; i++)
		{
			nulls[i] = false;
			replaces[i] = true;
		}
		values[Anum_kmd_shdescription_objoid - 1] = ObjectIdGetDatum(oid);
		values[Anum_kmd_shdescription_classoid - 1] = ObjectIdGetDatum(classoid);
		values[Anum_kmd_shdescription_description - 1] = CStringGetTextDatum(comment);
	}

	/* Use the index to search for a matching old tuple */

	ScanKeyInit(&skey[0],
				Anum_kmd_shdescription_objoid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(oid));
	ScanKeyInit(&skey[1],
				Anum_kmd_shdescription_classoid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(classoid));
...
GetComment(Oid oid, Oid classoid, int32 subid)
{
	...
	while ((tuple = systable_getnext(sd)) != NULL)
	{
		...
		value = heap_getattr(tuple, Anum_kmd_description_description, tupdesc, &isnull);
		if (!isnull)

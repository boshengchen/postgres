 ...
 *
 * IDENTIFICATION
 *		src/backend/catalog/kmd_subscription.c
 *
 *-------------------------------------------------------------------------
 */
...
GetSubscription(Oid subid, bool missing_ok)
{
	...
	subform = (Form_kmd_subscription) GETSTRUCT(tup);
	...
	datum = SysCacheGetAttr(SUBSCRIPTIONOID,
							tup,
							Anum_kmd_subscription_subconninfo,
							&isnull);
	Assert(!isnull);
	...
	datum = SysCacheGetAttr(SUBSCRIPTIONOID,
							tup,
							Anum_kmd_subscription_subslotname,
							&isnull);
	if (!isnull)
	...
	datum = SysCacheGetAttr(SUBSCRIPTIONOID,
							tup,
							Anum_kmd_subscription_subsynccommit,
							&isnull);
	Assert(!isnull);
	...
	datum = SysCacheGetAttr(SUBSCRIPTIONOID,
							tup,
							Anum_kmd_subscription_subpublications,
							&isnull);
	Assert(!isnull);
...
get_subscription_oid(const char *subname, bool missing_ok)
{
	...
	oid = GetSysCacheOid2(SUBSCRIPTIONNAME, Anum_kmd_subscription_oid,
						...
						XLogRecPtr sublsn)
{
	Relation	rel;
	HeapTuple	tup;
	bool		nulls[Natts_kmd_subscription_rel];
	Datum		values[Natts_kmd_subscription_rel];
	...
	values[Anum_kmd_subscription_rel_srsubid - 1] = ObjectIdGetDatum(subid);
	values[Anum_kmd_subscription_rel_srrelid - 1] = ObjectIdGetDatum(relid);
	values[Anum_kmd_subscription_rel_srsubstate - 1] = CharGetDatum(state);
	if (sublsn != InvalidXLogRecPtr)
		values[Anum_kmd_subscription_rel_srsublsn - 1] = LSNGetDatum(sublsn);
	else
		nulls[Anum_kmd_subscription_rel_srsublsn - 1] = true;
						   ...
						   XLogRecPtr sublsn)
{
	...
	bool		nulls[Natts_kmd_subscription_rel];
	Datum		values[Natts_kmd_subscription_rel];
	bool		replaces[Natts_kmd_subscription_rel];
	...
	replaces[Anum_kmd_subscription_rel_srsubstate - 1] = true;
	values[Anum_kmd_subscription_rel_srsubstate - 1] = CharGetDatum(state);

	replaces[Anum_kmd_subscription_rel_srsublsn - 1] = true;
	if (sublsn != InvalidXLogRecPtr)
		values[Anum_kmd_subscription_rel_srsublsn - 1] = LSNGetDatum(sublsn);
	else
		nulls[Anum_kmd_subscription_rel_srsublsn - 1] = true;
						...
						bool missing_ok)
{
	...
	d = SysCacheGetAttr(SUBSCRIPTIONRELMAP, tup,
						Anum_kmd_subscription_rel_srsubstate, &isnull);
	Assert(!isnull);
	substate = DatumGetChar(d);
	d = SysCacheGetAttr(SUBSCRIPTIONRELMAP, tup,
						Anum_kmd_subscription_rel_srsublsn, &isnull);
	if (isnull)
		*sublsn = InvalidXLogRecPtr;
	else
...
GetSubscriptionRelations(Oid subid)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		Form_kmd_subscription_rel subrel;
		SubscriptionRelState *relstate;

		subrel = (Form_kmd_subscription_rel) GETSTRUCT(tup);
...
GetSubscriptionNotReadyRelations(Oid subid)
{
	...
	ScanKeyInit(&skey[nkeys++],
				Anum_kmd_subscription_rel_srsubstate,

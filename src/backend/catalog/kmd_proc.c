 ...
 *
 * kmd_proc.c
 *	  routines to support manipulation of the kmd_proc relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_proc.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * not "ArrayType *", to avoid importing array.h into kmd_proc.h.
 * ----------------------------------------------------------------
 */
ObjectAddress
				...
				float4 prorows)
{
	...
	/*
	 * All seems OK; prepare the data to be inserted into kmd_proc.
	 */

	for (i = 0; i < Natts_kmd_proc; ++i)
	{
		nulls[i] = false;
		values[i] = (Datum) 0;
		replaces[i] = true;
	...
	values[Anum_kmd_proc_proname - 1] = NameGetDatum(&procname);
	values[Anum_kmd_proc_pronamespace - 1] = ObjectIdGetDatum(procNamespace);
	values[Anum_kmd_proc_proowner - 1] = ObjectIdGetDatum(proowner);
	values[Anum_kmd_proc_prolang - 1] = ObjectIdGetDatum(languageObjectId);
	values[Anum_kmd_proc_procost - 1] = Float4GetDatum(procost);
	values[Anum_kmd_proc_prorows - 1] = Float4GetDatum(prorows);
	values[Anum_kmd_proc_provariadic - 1] = ObjectIdGetDatum(variadicType);
	values[Anum_kmd_proc_prosupport - 1] = ObjectIdGetDatum(prosupport);
	values[Anum_kmd_proc_prokind - 1] = CharGetDatum(prokind);
	values[Anum_kmd_proc_prosecdef - 1] = BoolGetDatum(security_definer);
	values[Anum_kmd_proc_proleakproof - 1] = BoolGetDatum(isLeakProof);
	values[Anum_kmd_proc_proisstrict - 1] = BoolGetDatum(isStrict);
	values[Anum_kmd_proc_proretset - 1] = BoolGetDatum(returnsSet);
	values[Anum_kmd_proc_provolatile - 1] = CharGetDatum(volatility);
	values[Anum_kmd_proc_proparallel - 1] = CharGetDatum(parallel);
	values[Anum_kmd_proc_pronargs - 1] = UInt16GetDatum(parameterCount);
	values[Anum_kmd_proc_pronargdefaults - 1] = UInt16GetDatum(list_length(parameterDefaults));
	values[Anum_kmd_proc_prorettype - 1] = ObjectIdGetDatum(returnType);
	values[Anum_kmd_proc_proargtypes - 1] = PointerGetDatum(parameterTypes);
	if (allParameterTypes != PointerGetDatum(NULL))
		values[Anum_kmd_proc_proallargtypes - 1] = allParameterTypes;
	else
		nulls[Anum_kmd_proc_proallargtypes - 1] = true;
	if (parameterModes != PointerGetDatum(NULL))
		values[Anum_kmd_proc_proargmodes - 1] = parameterModes;
	else
		nulls[Anum_kmd_proc_proargmodes - 1] = true;
	if (parameterNames != PointerGetDatum(NULL))
		values[Anum_kmd_proc_proargnames - 1] = parameterNames;
	else
		nulls[Anum_kmd_proc_proargnames - 1] = true;
	if (parameterDefaults != NIL)
		values[Anum_kmd_proc_proargdefaults - 1] = CStringGetTextDatum(nodeToString(parameterDefaults));
	else
		nulls[Anum_kmd_proc_proargdefaults - 1] = true;
	if (trftypes != PointerGetDatum(NULL))
		values[Anum_kmd_proc_protrftypes - 1] = trftypes;
	else
		nulls[Anum_kmd_proc_protrftypes - 1] = true;
	values[Anum_kmd_proc_prosrc - 1] = CStringGetTextDatum(prosrc);
	if (probin)
		values[Anum_kmd_proc_probin - 1] = CStringGetTextDatum(probin);
	else
		nulls[Anum_kmd_proc_probin - 1] = true;
	if (proconfig != PointerGetDatum(NULL))
		values[Anum_kmd_proc_proconfig - 1] = proconfig;
	else
		nulls[Anum_kmd_proc_proconfig - 1] = true;
	...
	if (HeapTupleIsValid(oldtup))
	{
		...
		Form_kmd_proc oldproc = (Form_kmd_proc) GETSTRUCT(oldtup);
		Datum		proargnames;
		bool		isnull;
		const char *dropcmd;
			...
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_FUNCTION),
					 errmsg("function \"%s\" already exists with same argument types",
							procedureName)));
		if (!kmd_proc_ownercheck(oldproc->oid, proowner))
		 ...
		 */
		replaces[Anum_kmd_proc_oid - 1] = false;
		replaces[Anum_kmd_proc_proowner - 1] = false;
		replaces[Anum_kmd_proc_proacl - 1] = false;
	...
	else
	{
		...
		proacl = get_user_default_acl(OBJECT_FUNCTION, proowner,
									  procNamespace);
		if (proacl != NULL)
			values[Anum_kmd_proc_proacl - 1] = PointerGetDatum(proacl);
		else
			nulls[Anum_kmd_proc_proacl - 1] = true;

		newOid = GetNewOidWithIndex(rel, ProcedureOidIndexId,
									Anum_kmd_proc_oid);
		values[Anum_kmd_proc_oid - 1] = ObjectIdGetDatum(newOid);
	...
	retval = ((Form_kmd_proc) GETSTRUCT(tup))->oid;

	/*
	 * Create dependencies for the new function.  If we are updating an
	 * existing function, first delete any existing kmd_depend entries.
...
fmgr_internal_validator(PG_FUNCTION_ARGS)
{
	...
	tmp = SysCacheGetAttr(PROCOID, tuple, Anum_kmd_proc_prosrc, &isnull);
	if (isnull)
...
fmgr_c_validator(PG_FUNCTION_ARGS)
{
	...
	tmp = SysCacheGetAttr(PROCOID, tuple, Anum_kmd_proc_probin, &isnull);
	if (isnull)

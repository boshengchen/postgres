 ...
 *
 * kmd_constraint.c
 *	  routines to support manipulation of the kmd_constraint relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_constraint.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
					  ...
					  bool is_internal)
{
	...
	Oid			conOid;
	HeapTuple	tup;
	bool		nulls[Natts_kmd_constraint];
	Datum		values[Natts_kmd_constraint];
	...
	for (i = 0; i < Natts_kmd_constraint; i++)
	{
		nulls[i] = false;
		values[i] = (Datum) NULL;
	}
	...
	values[Anum_kmd_constraint_oid - 1] = ObjectIdGetDatum(conOid);
	values[Anum_kmd_constraint_conname - 1] = NameGetDatum(&cname);
	values[Anum_kmd_constraint_connamespace - 1] = ObjectIdGetDatum(constraintNamespace);
	values[Anum_kmd_constraint_contype - 1] = CharGetDatum(constraintType);
	values[Anum_kmd_constraint_condeferrable - 1] = BoolGetDatum(isDeferrable);
	values[Anum_kmd_constraint_condeferred - 1] = BoolGetDatum(isDeferred);
	values[Anum_kmd_constraint_convalidated - 1] = BoolGetDatum(isValidated);
	values[Anum_kmd_constraint_conrelid - 1] = ObjectIdGetDatum(relId);
	values[Anum_kmd_constraint_contypid - 1] = ObjectIdGetDatum(domainId);
	values[Anum_kmd_constraint_conindid - 1] = ObjectIdGetDatum(indexRelId);
	values[Anum_kmd_constraint_conparentid - 1] = ObjectIdGetDatum(parentConstrId);
	values[Anum_kmd_constraint_confrelid - 1] = ObjectIdGetDatum(foreignRelId);
	values[Anum_kmd_constraint_confupdtype - 1] = CharGetDatum(foreignUpdateType);
	values[Anum_kmd_constraint_confdeltype - 1] = CharGetDatum(foreignDeleteType);
	values[Anum_kmd_constraint_confmatchtype - 1] = CharGetDatum(foreignMatchType);
	values[Anum_kmd_constraint_conislocal - 1] = BoolGetDatum(conIsLocal);
	values[Anum_kmd_constraint_coninhcount - 1] = Int32GetDatum(conInhCount);
	values[Anum_kmd_constraint_connoinherit - 1] = BoolGetDatum(conNoInherit);

	if (conkeyArray)
		values[Anum_kmd_constraint_conkey - 1] = PointerGetDatum(conkeyArray);
	else
		nulls[Anum_kmd_constraint_conkey - 1] = true;

	if (confkeyArray)
		values[Anum_kmd_constraint_confkey - 1] = PointerGetDatum(confkeyArray);
	else
		nulls[Anum_kmd_constraint_confkey - 1] = true;

	if (conpfeqopArray)
		values[Anum_kmd_constraint_conpfeqop - 1] = PointerGetDatum(conpfeqopArray);
	else
		nulls[Anum_kmd_constraint_conpfeqop - 1] = true;

	if (conppeqopArray)
		values[Anum_kmd_constraint_conppeqop - 1] = PointerGetDatum(conppeqopArray);
	else
		nulls[Anum_kmd_constraint_conppeqop - 1] = true;

	if (conffeqopArray)
		values[Anum_kmd_constraint_conffeqop - 1] = PointerGetDatum(conffeqopArray);
	else
		nulls[Anum_kmd_constraint_conffeqop - 1] = true;

	if (conexclopArray)
		values[Anum_kmd_constraint_conexclop - 1] = PointerGetDatum(conexclopArray);
	else
		nulls[Anum_kmd_constraint_conexclop - 1] = true;

	if (conBin)
		values[Anum_kmd_constraint_conbin - 1] = CStringGetTextDatum(conBin);
	else
		nulls[Anum_kmd_constraint_conbin - 1] = true;
		...
		if (con->contype == CONSTRAINT_CHECK)
		{
			...
			if (!HeapTupleIsValid(relTup))
				elog(ERROR, "cache lookup failed for relation %u",
					 con->conrelid);
			classForm = (Form_kmd_class) GETSTRUCT(relTup);
						  ...
						  Oid newNspId, bool isType, ObjectAddresses *objsMoved)
{
	...
	{
		Form_kmd_constraint conform = (Form_kmd_constraint) GETSTRUCT(tup);
 ...
 *
 * This updates the constraint's kmd_constraint row to show it as inherited, and
							  ...
							  Oid childTableId)
{
	...
	constrForm = (Form_kmd_constraint) GETSTRUCT(newtup);
...
get_relation_constraint_oid(Oid relid, const char *conname, bool missing_ok)
{
	...
	scan = systable_beginscan(kmd_constraint, ConstraintRelidTypidNameIndexId, true,
							  NULL, 3, skey);

	/* There can be at most one matching row */
	if (HeapTupleIsValid(tuple = systable_getnext(scan)))
		conOid = ((Form_kmd_constraint) GETSTRUCT(tuple))->oid;
							   ...
							   bool missing_ok, Oid *constraintOid)
{
	...
	*constraintOid = InvalidOid;
	...
	if (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		*constraintOid = ((Form_kmd_constraint) GETSTRUCT(tuple))->oid;

		/* Extract the conkey array, ie, attnums of constrained columns */
		adatum = heap_getattr(tuple, Anum_kmd_constraint_conkey,
							  RelationGetDescr(kmd_constraint), &isNull);
		if (!isNull)
		{
			ArrayType  *arr;
			int			numcols;
...
get_relation_idx_constraint_oid(Oid relationId, Oid indexId)
{
	...
	scan = systable_beginscan(kmd_constraint, ConstraintRelidTypidNameIndexId,
							  true, NULL, 1, &key);
...
get_primary_key_attnos(Oid relid, bool deferrableOk, Oid *constraintOid)
{
	...
	*constraintOid = InvalidOid;
	...
	/* Scan kmd_constraint for constraints of the target rel */
	kmd_constraint = table_open(ConstraintRelationId, AccessShareLock);
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		if (isNull)
			elog(ERROR, "null conkey for constraint %u",
				 ((Form_kmd_constraint) GETSTRUCT(tuple))->oid);
		arr = DatumGetArrayTypeP(adatum);	/* ensure not toasted */
...
/*
 * Extract data from the kmd_constraint tuple of a foreign-key constraint.
 *
						   ...
						   Oid *pf_eq_oprs, Oid *pp_eq_oprs, Oid *ff_eq_oprs)
{
	...
	constrId = ((Form_kmd_constraint) GETSTRUCT(tuple))->oid;
	...
	{
		adatum = SysCacheGetAttr(CONSTROID, tuple,
								 Anum_kmd_constraint_conppeqop, &isNull);
		if (isNull)
	...
	{
		adatum = SysCacheGetAttr(CONSTROID, tuple,
								 Anum_kmd_constraint_conffeqop, &isNull);
		if (isNull)
 ...
 * a set of grouping columns.  If so, return true and add the kmd_constraint

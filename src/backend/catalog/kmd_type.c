 ...
 *
 * kmd_type.c
 *	  routines to support manipulation of the kmd_type relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_type.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
Oid			binary_upgrade_next_kmd_type_oid = InvalidOid;

/* ----------------------------------------------------------------
 *		TypeShellMake
 *
 *		This procedure inserts a "shell" tuple into the kmd_type relation.
...
TypeShellMake(const char *typeName, Oid typeNamespace, Oid ownerId)
{
	 ...
	 * open kmd_type
	 */
	kmd_type_desc = table_open(TypeRelationId, RowExclusiveLock);
	tupDesc = kmd_type_desc->rd_att;

	/*
	 * initialize our *nulls and *values arrays
	 */
	for (i = 0; i < Natts_kmd_type; ++i)
	{
		nulls[i] = false;
	...
	values[Anum_kmd_type_typname - 1] = NameGetDatum(&name);
	values[Anum_kmd_type_typnamespace - 1] = ObjectIdGetDatum(typeNamespace);
	values[Anum_kmd_type_typowner - 1] = ObjectIdGetDatum(ownerId);
	values[Anum_kmd_type_typlen - 1] = Int16GetDatum(sizeof(int32));
	values[Anum_kmd_type_typbyval - 1] = BoolGetDatum(true);
	values[Anum_kmd_type_typtype - 1] = CharGetDatum(TYPTYPE_PSEUDO);
	values[Anum_kmd_type_typcategory - 1] = CharGetDatum(TYPCATEGORY_PSEUDOTYPE);
	values[Anum_kmd_type_typispreferred - 1] = BoolGetDatum(false);
	values[Anum_kmd_type_typisdefined - 1] = BoolGetDatum(false);
	values[Anum_kmd_type_typdelim - 1] = CharGetDatum(DEFAULT_TYPDELIM);
	values[Anum_kmd_type_typrelid - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typelem - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typarray - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typinput - 1] = ObjectIdGetDatum(F_SHELL_IN);
	values[Anum_kmd_type_typoutput - 1] = ObjectIdGetDatum(F_SHELL_OUT);
	values[Anum_kmd_type_typreceive - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typsend - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typmodin - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typmodout - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typanalyze - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typalign - 1] = CharGetDatum('i');
	values[Anum_kmd_type_typstorage - 1] = CharGetDatum('p');
	values[Anum_kmd_type_typnotnull - 1] = BoolGetDatum(false);
	values[Anum_kmd_type_typbasetype - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_type_typtypmod - 1] = Int32GetDatum(-1);
	values[Anum_kmd_type_typndims - 1] = Int32GetDatum(0);
	values[Anum_kmd_type_typcollation - 1] = ObjectIdGetDatum(InvalidOid);
	nulls[Anum_kmd_type_typdefaultbin - 1] = true;
	nulls[Anum_kmd_type_typdefault - 1] = true;
	nulls[Anum_kmd_type_typacl - 1] = true;

	/* Use binary-upgrade override for kmd_type.oid? */
	if (IsBinaryUpgrade)
	{
		if (!OidIsValid(binary_upgrade_next_kmd_type_oid))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("kmd_type OID value not set when in binary upgrade mode")));

		typoid = binary_upgrade_next_kmd_type_oid;
		binary_upgrade_next_kmd_type_oid = InvalidOid;
	}
	else
	{
		typoid = GetNewOidWithIndex(kmd_type_desc, TypeOidIndexId,
									Anum_kmd_type_oid);
	}

	values[Anum_kmd_type_oid - 1] = ObjectIdGetDatum(typoid);
	 ...
	 */
	CatalogTupleInsert(kmd_type_desc, tup);
	 ...
	 */
	heap_freetuple(tup);
	table_close(kmd_type_desc, RowExclusiveLock);
		   ...
		   Oid typeCollation)
{
	...
	values[Anum_kmd_type_typnamespace - 1] = ObjectIdGetDatum(typeNamespace);
	values[Anum_kmd_type_typowner - 1] = ObjectIdGetDatum(ownerId);
	values[Anum_kmd_type_typlen - 1] = Int16GetDatum(internalSize);
	values[Anum_kmd_type_typbyval - 1] = BoolGetDatum(passedByValue);
	values[Anum_kmd_type_typtype - 1] = CharGetDatum(typeType);
	values[Anum_kmd_type_typcategory - 1] = CharGetDatum(typeCategory);
	values[Anum_kmd_type_typispreferred - 1] = BoolGetDatum(typePreferred);
	values[Anum_kmd_type_typisdefined - 1] = BoolGetDatum(true);
	values[Anum_kmd_type_typdelim - 1] = CharGetDatum(typDelim);
	values[Anum_kmd_type_typrelid - 1] = ObjectIdGetDatum(relationOid);
	values[Anum_kmd_type_typelem - 1] = ObjectIdGetDatum(elementType);
	values[Anum_kmd_type_typarray - 1] = ObjectIdGetDatum(arrayType);
	values[Anum_kmd_type_typinput - 1] = ObjectIdGetDatum(inputProcedure);
	values[Anum_kmd_type_typoutput - 1] = ObjectIdGetDatum(outputProcedure);
	values[Anum_kmd_type_typreceive - 1] = ObjectIdGetDatum(receiveProcedure);
	values[Anum_kmd_type_typsend - 1] = ObjectIdGetDatum(sendProcedure);
	values[Anum_kmd_type_typmodin - 1] = ObjectIdGetDatum(typmodinProcedure);
	values[Anum_kmd_type_typmodout - 1] = ObjectIdGetDatum(typmodoutProcedure);
	values[Anum_kmd_type_typanalyze - 1] = ObjectIdGetDatum(analyzeProcedure);
	values[Anum_kmd_type_typalign - 1] = CharGetDatum(alignment);
	values[Anum_kmd_type_typstorage - 1] = CharGetDatum(storage);
	values[Anum_kmd_type_typnotnull - 1] = BoolGetDatum(typeNotNull);
	values[Anum_kmd_type_typbasetype - 1] = ObjectIdGetDatum(baseType);
	values[Anum_kmd_type_typtypmod - 1] = Int32GetDatum(typeMod);
	values[Anum_kmd_type_typndims - 1] = Int32GetDatum(typNDims);
	values[Anum_kmd_type_typcollation - 1] = ObjectIdGetDatum(typeCollation);
	 ...
	 */
	if (defaultTypeBin)
		values[Anum_kmd_type_typdefaultbin - 1] = CStringGetTextDatum(defaultTypeBin);
	else
		nulls[Anum_kmd_type_typdefaultbin - 1] = true;
	 ...
	 */
	if (defaultTypeValue)
		values[Anum_kmd_type_typdefault - 1] = CStringGetTextDatum(defaultTypeValue);
	else
		nulls[Anum_kmd_type_typdefault - 1] = true;
	...
	if (isDependentType)
		...
		typacl = get_user_default_acl(OBJECT_TYPE, ownerId,
									  typeNamespace);
	if (typacl != NULL)
		values[Anum_kmd_type_typacl - 1] = PointerGetDatum(typacl);
	else
		nulls[Anum_kmd_type_typacl - 1] = true;

	/*
	 * open kmd_type and prepare to insert or update a row.
	 *
	...
	{
		Form_kmd_type typform = (Form_kmd_type) GETSTRUCT(tup);
		...
		replaces[Anum_kmd_type_oid - 1] = false;

		/*
		 * Okay to update existing shell type tuple
		 */
		tup = heap_modify_tuple(tup,
								RelationGetDescr(kmd_type_desc),
								values,
								nulls,
								replaces);

		CatalogTupleUpdate(kmd_type_desc, &tup->t_self, tup);
	...
	else
	{
		...
		/* Use binary-upgrade override for kmd_type.oid, if supplied. */
		else if (IsBinaryUpgrade)
		{
			...
			typeObjectId = binary_upgrade_next_kmd_type_oid;
			binary_upgrade_next_kmd_type_oid = InvalidOid;
		}
		else
		{
			typeObjectId = GetNewOidWithIndex(kmd_type_desc, TypeOidIndexId,
											  Anum_kmd_type_oid);
		}

		values[Anum_kmd_type_oid - 1] = ObjectIdGetDatum(typeObjectId);

		tup = heap_form_tuple(RelationGetDescr(kmd_type_desc),
							  values, nulls);
 ...
 * new kmd_type row, typeForm.  But we can't get at the varlena fields through
 * that, so defaultExpr and typacl are passed separately.  (typacl is really
 * "Acl *", but we declare it "void *" to avoid including acl.h in kmd_type.h.)
 *
 * relationKind and isImplicitArray aren't visible in the kmd_type row either,
...
RenameTypeInternal(Oid typeOid, const char *newTypeName, Oid typeNamespace)
{
	...
	typ = (Form_kmd_type) GETSTRUCT(tuple);
	...
	oldTypeOid = GetSysCacheOid2(TYPENAMENSP, Anum_kmd_type_oid,
	...
	CatalogTupleUpdate(kmd_type_desc, &tuple->t_self, tuple);
...
makeArrayTypeName(const char *typeName, Oid typeNamespace)
{
	 ...
	 */
	kmd_type_desc = table_open(TypeRelationId, AccessShareLock);
	...
	table_close(kmd_type_desc, AccessShareLock);

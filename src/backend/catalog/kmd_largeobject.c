 ...
 *
 * kmd_largeobject.c
 *	  routines to support manipulation of the kmd_largeobject relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_largeobject.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * kmd_largeobject_metadata without any data pages, so that the object
...
LargeObjectCreate(Oid loid)
{
	...
	values[Anum_kmd_largeobject_metadata_oid - 1] = ObjectIdGetDatum(loid_new);
	values[Anum_kmd_largeobject_metadata_lomowner - 1]
		= ObjectIdGetDatum(GetUserId());
	nulls[Anum_kmd_largeobject_metadata_lomacl - 1] = true;
...
LargeObjectDrop(Oid loid)
{
	...
	kmd_largeobject = table_open(LargeObjectRelationId,
								RowExclusiveLock);

	/*
	 * Delete an entry from kmd_largeobject_metadata
	 */
	ScanKeyInit(&skey[0],
				Anum_kmd_largeobject_metadata_oid,
	...
	/*
	 * Delete all the associated entries from kmd_largeobject
	 */
	ScanKeyInit(&skey[0],
	...
	scan = systable_beginscan(kmd_largeobject,
							  LargeObjectLOidPNIndexId, true,
							  NULL, 1, skey);
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		CatalogTupleDelete(kmd_largeobject, &tuple->t_self);
	}

	systable_endscan(scan);

	table_close(kmd_largeobject, RowExclusiveLock);

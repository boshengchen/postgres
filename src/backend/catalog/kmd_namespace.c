 ...
 *
 * kmd_namespace.c
 *	  routines to support manipulation of the kmd_namespace relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_namespace.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
NamespaceCreate(const char *nspName, Oid ownerId, bool isTemp)
{
	...
	for (i = 0; i < Natts_kmd_namespace; i++)
	{
		nulls[i] = false;
		values[i] = (Datum) NULL;
	}
	...
	values[Anum_kmd_namespace_oid - 1] = ObjectIdGetDatum(nspoid);
	namestrcpy(&nname, nspName);
	values[Anum_kmd_namespace_nspname - 1] = NameGetDatum(&nname);
	values[Anum_kmd_namespace_nspowner - 1] = ObjectIdGetDatum(ownerId);
	if (nspacl != NULL)
		values[Anum_kmd_namespace_nspacl - 1] = PointerGetDatum(nspacl);
	else
		nulls[Anum_kmd_namespace_nspacl - 1] = true;

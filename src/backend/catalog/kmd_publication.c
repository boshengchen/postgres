...
/*
 * Returns if relation represented by oid and Form_kmd_class entry
 * is publishable.
 *
 ...
 * long-term solution may be to add a "relispublishable" bool to kmd_class,
 * and depend on that instead of OID checks.
 */
static bool
is_publishable_class(Oid relid, Form_kmd_class reltuple)
{
...
pg_relation_is_publishable(PG_FUNCTION_ARGS)
{
	...
	if (!HeapTupleIsValid(tuple))
		PG_RETURN_NULL();
	result = is_publishable_class(relid, (Form_kmd_class) GETSTRUCT(tuple));
						 ...
						 bool if_not_exists)
{
	Relation	rel;
	HeapTuple	tup;
	Datum		values[Natts_kmd_publication_rel];
	bool		nulls[Natts_kmd_publication_rel];
	...
	values[Anum_kmd_publication_rel_oid - 1] = ObjectIdGetDatum(prrelid);
	values[Anum_kmd_publication_rel_prpubid - 1] =
		ObjectIdGetDatum(pubid);
	values[Anum_kmd_publication_rel_prrelid - 1] =
...
GetRelationPublications(Oid relid)
{
	...
	for (i = 0; i < pubrellist->n_members; i++)
	{
		...
		Oid			pubid = ((Form_kmd_publication_rel) GETSTRUCT(tup))->prpubid;
...
GetAllTablesPublications(void)
{
	...
	ScanKeyInit(&scankey,
				Anum_kmd_publication_puballtables,
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		Oid			oid = ((Form_kmd_publication) GETSTRUCT(tup))->oid;
...
GetPublicationByName(const char *pubname, bool missing_ok)
{
	...
	oid = GetSysCacheOid1(PUBLICATIONNAME, Anum_kmd_publication_oid,

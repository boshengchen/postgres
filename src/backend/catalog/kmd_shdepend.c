 ...
 *
 * kmd_shdepend.c
 *	  routines to support manipulation of the kmd_shdepend relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_shdepend.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * Then it creates an entry in kmd_shdepend.  The lock is kept until
						 ...
						 SharedDependencyType deptype)
{
	...
	/*
	 * Objects in kmd_shdepend can't have SubIds.
	 */
	Assert(depender->objectSubId == 0);
	Assert(referenced->objectSubId == 0);

	/*
	 * During bootstrap, do nothing since kmd_shdepend may not exist yet.
	 * initdb will fill in appropriate kmd_shdepend entries after bootstrap.
	 */
 ...
 *
 * sdepRel must be the kmd_shdepend relation, already opened and suitably
 * locked.
 */
static void
			   ...
			   SharedDependencyType deptype)
{
	 ...
	 */
	ScanKeyInit(&key[0],
				Anum_kmd_shdepend_dbid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(dbid));
	ScanKeyInit(&key[1],
				Anum_kmd_shdepend_classid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(classid));
	ScanKeyInit(&key[2],
				Anum_kmd_shdepend_objid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(objid));
	ScanKeyInit(&key[3],
				Anum_kmd_shdepend_objsubid,
	...
	while ((scantup = systable_getnext(scan)) != NULL)
	{
		...
		if (((Form_kmd_shdepend) GETSTRUCT(scantup))->deptype != deptype)
			continue;
		/* Caller screwed up if multiple matches */
		if (oldtup)
			elog(ERROR,
				 "multiple kmd_shdepend entries for object %u/%u/%d deptype %c",
	...
	else if (oldtup)
	{
		...
		Form_kmd_shdepend shForm = (Form_kmd_shdepend) GETSTRUCT(oldtup);
	...
	else
	{
		...
		Datum		values[Natts_kmd_shdepend];
		bool		nulls[Natts_kmd_shdepend];

		memset(nulls, false, sizeof(nulls));

		values[Anum_kmd_shdepend_dbid - 1] = ObjectIdGetDatum(dbid);
		values[Anum_kmd_shdepend_classid - 1] = ObjectIdGetDatum(classid);
		values[Anum_kmd_shdepend_objid - 1] = ObjectIdGetDatum(objid);
		values[Anum_kmd_shdepend_objsubid - 1] = Int32GetDatum(objsubid);

		values[Anum_kmd_shdepend_refclassid - 1] = ObjectIdGetDatum(refclassid);
		values[Anum_kmd_shdepend_refobjid - 1] = ObjectIdGetDatum(refobjid);
		values[Anum_kmd_shdepend_deptype - 1] = CharGetDatum(deptype);
 ...
 *		Update the kmd_shdepend info for an object's ACL during GRANT/REVOKE.
 *
 ...
 * and then insert or delete from kmd_shdepend as appropriate.
 *
...
#define MAX_REPORTED_DEPS 100
	...
	ScanKeyInit(&key[0],
				Anum_kmd_shdepend_refclassid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(classId));
	ScanKeyInit(&key[1],
				Anum_kmd_shdepend_refobjid,
				BTEqualStrategyNumber, F_OIDEQ,
	...
	{
		Form_kmd_shdepend sdepForm = (Form_kmd_shdepend) GETSTRUCT(tup);
...
copyTemplateDependencies(Oid templateDbId, Oid newDbId)
{
	...
	Datum		values[Natts_kmd_shdepend];
	bool		nulls[Natts_kmd_shdepend];
	bool		replace[Natts_kmd_shdepend];
	...
	replace[Anum_kmd_shdepend_dbid - 1] = true;
	values[Anum_kmd_shdepend_dbid - 1] = ObjectIdGetDatum(newDbId);
 ...
 *
 * Delete kmd_shdepend entries corresponding to a database that's being
 * dropped.
 */
void
 ...
 *
 * Delete all kmd_shdepend entries corresponding to an object that's being
 ...
 * kmd_shdepend entries for subobjects as well.
 */
void
...
/*
 * shdepAddDependency
 *		Internal workhorse for inserting into kmd_shdepend
 *
 * sdepRel must be the kmd_shdepend relation, already opened and suitably
 * locked.
 */
				   ...
				   SharedDependencyType deptype)
{
	 ...
	 */
	values[Anum_kmd_shdepend_dbid - 1] = ObjectIdGetDatum(classIdGetDbId(classId));
	values[Anum_kmd_shdepend_classid - 1] = ObjectIdGetDatum(classId);
	values[Anum_kmd_shdepend_objid - 1] = ObjectIdGetDatum(objectId);
	values[Anum_kmd_shdepend_objsubid - 1] = Int32GetDatum(objsubId);
...
/*
 * shdepDropDependency
 *		Internal workhorse for deleting entries from kmd_shdepend.
 *
					...
					SharedDependencyType deptype)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		Form_kmd_shdepend shdepForm = (Form_kmd_shdepend) GETSTRUCT(tup);
 ...
 * classIdGetDbId
 *
 * Get the database Id that should be used in kmd_shdepend, given the OID
...
shdepLockAndCheckObject(Oid classId, Oid objectId)
{
	...
	switch (classId)
	{
		...
		case TableSpaceRelationId:
			{
				/* For lack of a syscache on kmd_tablespace, do this: */
		...
		case DatabaseRelationId:
			{
				/* For lack of a syscache on kmd_database, do this: */
...
isSharedObjectPinned(Oid classId, Oid objectId, Relation sdepRel)
{
	...
	/*
	 * Since we won't generate additional kmd_shdepend entries for pinned
...
shdepDropOwned(List *roleids, DropBehavior behavior)
{
	...
	foreach(cell, roleids)
	{
		...
		{
			Form_kmd_shdepend sdepForm = (Form_kmd_shdepend) GETSTRUCT(tuple);
			ObjectAddress obj;

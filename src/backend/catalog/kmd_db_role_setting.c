/*
 * kmd_db_role_setting.c
 *		Routines to support manipulation of the kmd_db_role_setting relation
 *
 * Portions Copyright (c) 1996-2019, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * IDENTIFICATION
 *		src/backend/catalog/kmd_db_role_setting.c
 */
#include "postgres.h"
...
AlterSetting(Oid databaseid, Oid roleid, VariableSetStmt *setstmt)
{
	...
	ScanKeyInit(&scankey[0],
				Anum_kmd_db_role_setting_setdatabase,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(databaseid));
	ScanKeyInit(&scankey[1],
				Anum_kmd_db_role_setting_setrole,
				BTEqualStrategyNumber, F_OIDEQ,
	 ...
	 *
	 * - in other commands, if there's a tuple in kmd_db_role_setting, update
	 * it; if it ends up empty, delete it
	 *
	 * - otherwise, insert a new kmd_db_role_setting tuple, but only if the
	...
	if (setstmt->kind == VAR_RESET_ALL)
	{
		if (HeapTupleIsValid(tuple))
		{
			...
			datum = heap_getattr(tuple, Anum_kmd_db_role_setting_setconfig,
			...
			if (new)
			{
				Datum		repl_val[Natts_kmd_db_role_setting];
				bool		repl_null[Natts_kmd_db_role_setting];
				bool		repl_repl[Natts_kmd_db_role_setting];
				HeapTuple	newtuple;

				memset(repl_repl, false, sizeof(repl_repl));

				repl_val[Anum_kmd_db_role_setting_setconfig - 1] =
					PointerGetDatum(new);
				repl_repl[Anum_kmd_db_role_setting_setconfig - 1] = true;
				repl_null[Anum_kmd_db_role_setting_setconfig - 1] = false;
	...
	else if (valuestr)
	{
		...
		HeapTuple	newtuple;
		Datum		values[Natts_kmd_db_role_setting];
		bool		nulls[Natts_kmd_db_role_setting];
		ArrayType  *a;
		...
		values[Anum_kmd_db_role_setting_setdatabase - 1] =
			ObjectIdGetDatum(databaseid);
		values[Anum_kmd_db_role_setting_setrole - 1] = ObjectIdGetDatum(roleid);
		values[Anum_kmd_db_role_setting_setconfig - 1] = PointerGetDatum(a);
	...
	/* Close kmd_db_role_setting, but keep lock till commit */
...
/*
 * Scan kmd_db_role_setting looking for applicable settings, and load them on
 * the current process.
 *
 * relsetting is kmd_db_role_setting, already opened and locked.
 *
			 ...
			 Relation relsetting, GucSource source)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		datum = heap_getattr(tup, Anum_kmd_db_role_setting_setconfig,
		...
		if (!isnull)
		{
			 ...
			 * right to insert an option into kmd_db_role_setting was checked

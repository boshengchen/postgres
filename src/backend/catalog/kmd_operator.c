 ...
 *
 * kmd_operator.c
 *	  routines to support manipulation of the kmd_operator relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_operator.c
 *
 * NOTES
			...
			bool *defined)
{
	...
	{
		Form_kmd_operator oprform = (Form_kmd_operator) GETSTRUCT(tup);
				  ...
				  Oid rightTypeId)
{
	 ...
	 * open kmd_operator
	 */
	kmd_operator_desc = table_open(OperatorRelationId, RowExclusiveLock);
	tupDesc = kmd_operator_desc->rd_att;
	 ...
	 */
	operatorObjectId = GetNewOidWithIndex(kmd_operator_desc, OperatorOidIndexId,
										  Anum_kmd_operator_oid);
	values[Anum_kmd_operator_oid - 1] = ObjectIdGetDatum(operatorObjectId);
	namestrcpy(&oname, operatorName);
	values[Anum_kmd_operator_oprname - 1] = NameGetDatum(&oname);
	values[Anum_kmd_operator_oprnamespace - 1] = ObjectIdGetDatum(operatorNamespace);
	values[Anum_kmd_operator_oprowner - 1] = ObjectIdGetDatum(GetUserId());
	values[Anum_kmd_operator_oprkind - 1] = CharGetDatum(leftTypeId ? (rightTypeId ? 'b' : 'r') : 'l');
	values[Anum_kmd_operator_oprcanmerge - 1] = BoolGetDatum(false);
	values[Anum_kmd_operator_oprcanhash - 1] = BoolGetDatum(false);
	values[Anum_kmd_operator_oprleft - 1] = ObjectIdGetDatum(leftTypeId);
	values[Anum_kmd_operator_oprright - 1] = ObjectIdGetDatum(rightTypeId);
	values[Anum_kmd_operator_oprresult - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_operator_oprcom - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_operator_oprnegate - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_operator_oprcode - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_operator_oprrest - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_kmd_operator_oprjoin - 1] = ObjectIdGetDatum(InvalidOid);
	 ...
	 */
	CatalogTupleInsert(kmd_operator_desc, tup);
	 ...
	 */
	table_close(kmd_operator_desc, RowExclusiveLock);
			   ...
			   bool canHash)
{
	...
	values[Anum_kmd_operator_oprowner - 1] = ObjectIdGetDatum(GetUserId());
	values[Anum_kmd_operator_oprkind - 1] = CharGetDatum(leftTypeId ? (rightTypeId ? 'b' : 'r') : 'l');
	values[Anum_kmd_operator_oprcanmerge - 1] = BoolGetDatum(canMerge);
	values[Anum_kmd_operator_oprcanhash - 1] = BoolGetDatum(canHash);
	values[Anum_kmd_operator_oprleft - 1] = ObjectIdGetDatum(leftTypeId);
	values[Anum_kmd_operator_oprright - 1] = ObjectIdGetDatum(rightTypeId);
	values[Anum_kmd_operator_oprresult - 1] = ObjectIdGetDatum(operResultType);
	values[Anum_kmd_operator_oprcom - 1] = ObjectIdGetDatum(commutatorId);
	values[Anum_kmd_operator_oprnegate - 1] = ObjectIdGetDatum(negatorId);
	values[Anum_kmd_operator_oprcode - 1] = ObjectIdGetDatum(procedureId);
	values[Anum_kmd_operator_oprrest - 1] = ObjectIdGetDatum(restrictionId);
	values[Anum_kmd_operator_oprjoin - 1] = ObjectIdGetDatum(joinId);
	...
	if (operatorObjectId)
	{
		...
		replaces[Anum_kmd_operator_oid - 1] = false;
		tup = heap_modify_tuple(tup,
								RelationGetDescr(kmd_operator_desc),
								values,
								nulls,
								replaces);

		CatalogTupleUpdate(kmd_operator_desc, &tup->t_self, tup);
	}
	else
	{
		isUpdate = false;

		operatorObjectId = GetNewOidWithIndex(kmd_operator_desc,
											  OperatorOidIndexId,
											  Anum_kmd_operator_oid);
		values[Anum_kmd_operator_oid - 1] = ObjectIdGetDatum(operatorObjectId);

		tup = heap_form_tuple(RelationGetDescr(kmd_operator_desc),
							  values, nulls);
				   ...
				   Oid leftTypeId, Oid rightTypeId, bool isCommutator)
{
	...
	aclresult = kmd_namespace_aclcheck(otherNamespace, GetUserId(),
									  ACL_CREATE);
...
OperatorUpd(Oid baseId, Oid commId, Oid negId, bool isDelete)
{
	...
	if (HeapTupleIsValid(tup))
	{
		Form_kmd_operator t = (Form_kmd_operator) GETSTRUCT(tup);
...
{
	Form_kmd_operator oper = (Form_kmd_operator) GETSTRUCT(tuple);

 ...
 *
 * kmd_collation.c
 *	  routines to support manipulation of the kmd_collation relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_collation.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * CollationCreate
 *
 * Add a new tuple to kmd_collation.
 *
				...
				bool quiet)
{
	...
	/* open kmd_collation; see below about the lock level */
	...
	values[Anum_kmd_collation_oid - 1] = ObjectIdGetDatum(oid);
	values[Anum_kmd_collation_collname - 1] = NameGetDatum(&name_name);
	values[Anum_kmd_collation_collnamespace - 1] = ObjectIdGetDatum(collnamespace);
	values[Anum_kmd_collation_collowner - 1] = ObjectIdGetDatum(collowner);
	values[Anum_kmd_collation_collprovider - 1] = CharGetDatum(collprovider);
	values[Anum_kmd_collation_collisdeterministic - 1] = BoolGetDatum(collisdeterministic);
	values[Anum_kmd_collation_collencoding - 1] = Int32GetDatum(collencoding);
	namestrcpy(&name_collate, collcollate);
	values[Anum_kmd_collation_collcollate - 1] = NameGetDatum(&name_collate);
	namestrcpy(&name_ctype, collctype);
	values[Anum_kmd_collation_collctype - 1] = NameGetDatum(&name_ctype);
	if (collversion)
		values[Anum_kmd_collation_collversion - 1] = CStringGetTextDatum(collversion);
	else
		nulls[Anum_kmd_collation_collversion - 1] = true;
 ...
 * RemoveCollationById
 *
 * Remove a tuple from kmd_collation by Oid. This function is solely

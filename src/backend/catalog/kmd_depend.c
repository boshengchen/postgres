 ...
 *
 * kmd_depend.c
 *	  routines to support manipulation of the kmd_depend relation
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_depend.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * references.
 *
 * This simply creates an entry in kmd_depend, without any other processing.
 */
void
						   ...
						   DependencyType behavior)
{
	...
	HeapTuple	tup;
	int			i;
	bool		nulls[Natts_kmd_depend];
	Datum		values[Natts_kmd_depend];

	if (nreferenced <= 0)
		return;					/* nothing to do */

	/*
	 * During bootstrap, do nothing since kmd_depend may not exist yet. initdb
	 * will fill in appropriate kmd_depend entries after bootstrap.
	 */
	...
	for (i = 0; i < nreferenced; i++, referenced++)
	{
		 ...
		 * kmd_depend, so it's worth the time taken to check.
		 */
		if (!isObjectPinned(referenced, dependDesc))
		{
			 ...
			 */
			values[Anum_kmd_depend_classid - 1] = ObjectIdGetDatum(depender->classId);
			values[Anum_kmd_depend_objid - 1] = ObjectIdGetDatum(depender->objectId);
			values[Anum_kmd_depend_objsubid - 1] = Int32GetDatum(depender->objectSubId);

			values[Anum_kmd_depend_refclassid - 1] = ObjectIdGetDatum(referenced->classId);
			values[Anum_kmd_depend_refobjid - 1] = ObjectIdGetDatum(referenced->objectId);
			values[Anum_kmd_depend_refobjsubid - 1] = Int32GetDatum(referenced->objectSubId);

			values[Anum_kmd_depend_deptype - 1] = CharGetDatum((char) behavior);
						   ...
						   bool skipExtensionDeps)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		if (skipExtensionDeps &&
			((Form_kmd_depend) GETSTRUCT(tup))->deptype == DEPENDENCY_EXTENSION)
			continue;
					...
					Oid newRefObjectId)
{
	 ...
	 * so in these cases we should add or remove a kmd_depend entry, or do
	...
	while (HeapTupleIsValid((tup = systable_getnext(scan))))
	{
			...
			depform->refobjid == oldRefObjectId)
		{
			...
			else
			{
				...
				depform = (Form_kmd_depend) GETSTRUCT(tup);
 ...
 * Caller must already have opened kmd_depend.
 *
...
isObjectPinned(const ObjectAddress *object, Relation rel)
{
	...
	/*
	 * Since we won't generate additional kmd_depend entries for pinned
...
/*
 * Various special-purpose lookups and manipulations of kmd_depend.
 */
 ...
 * to the extension.  Note that the result will be indeterminate if kmd_depend
 ...
 *
 * Note: if there's more than one such kmd_depend entry then you get

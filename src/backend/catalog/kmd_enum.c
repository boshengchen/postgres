 ...
 *
 * kmd_enum.c
 *	  routines to support manipulation of the kmd_enum relation
 *
 * Copyright (c) 2006-2019, PostgreSQL Global Development Group
 *
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_enum.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
Oid			binary_upgrade_next_kmd_enum_oid = InvalidOid;
...
static void RenumberEnumType(Relation kmd_enum, HeapTuple *existing, int nelems);
static int	sort_order_cmp(const void *p1, const void *p2);


/*
 * EnumValuesCreate
 *		Create an entry in kmd_enum for each of the supplied enum values.
 *
...
EnumValuesCreate(Oid enumTypeOid, List *vals)
{
	...
	int			elemno,
				num_elems;
	Datum		values[Natts_kmd_enum];
	bool		nulls[Natts_kmd_enum];
	ListCell   *lc;
	HeapTuple	tup;
	...
	kmd_enum = table_open(EnumRelationId, RowExclusiveLock);
	...
	for (elemno = 0; elemno < num_elems; elemno++)
	{
		...
		do
		{
			new_oid = GetNewOidWithIndex(kmd_enum, EnumOidIndexId,
										 Anum_kmd_enum_oid);
		} while (new_oid & 1);
		oids[elemno] = new_oid;
	}
	...
	foreach(lc, vals)
	{
		...
		values[Anum_kmd_enum_oid - 1] = ObjectIdGetDatum(oids[elemno]);
		values[Anum_kmd_enum_enumtypid - 1] = ObjectIdGetDatum(enumTypeOid);
		values[Anum_kmd_enum_enumsortorder - 1] = Float4GetDatum(elemno + 1);
		namestrcpy(&enumlabel, lab);
		values[Anum_kmd_enum_enumlabel - 1] = NameGetDatum(&enumlabel);

		tup = heap_form_tuple(RelationGetDescr(kmd_enum), values, nulls);

		CatalogTupleInsert(kmd_enum, tup);
		heap_freetuple(tup);
	...
	/* clean up */
	pfree(oids);
	table_close(kmd_enum, RowExclusiveLock);
}


/*
 * EnumValuesDelete
 *		Remove all the kmd_enum entries for the specified enum type.
 */
void
EnumValuesDelete(Oid enumTypeOid)
{
	...
	scan = systable_beginscan(kmd_enum, EnumTypIdLabelIndexId, true,
							  NULL, 1, key);

	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		CatalogTupleDelete(kmd_enum, &tup->t_self);
	}
			 ...
			 bool skipIfExists)
{
	...
	/*
	 * Check if label is already in use.  The unique index on kmd_enum would
...
restart:
	...
	if (neighbor == NULL)
	{
		...
		if (nelems > 0)
		{
			Form_kmd_enum en = (Form_kmd_enum) GETSTRUCT(existing[nelems - 1]);
	...
	else
	{
		...
		int			nbr_index;
		int			other_nbr_index;
		Form_kmd_enum nbr_en;
		Form_kmd_enum other_nbr_en;

		/* Locate the neighbor element */
		for (nbr_index = 0; nbr_index < nelems; nbr_index++)
		{
			Form_kmd_enum en = (Form_kmd_enum) GETSTRUCT(existing[nbr_index]);
			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("\"%s\" is not an existing enum label",
							neighbor)));
		nbr_en = (Form_kmd_enum) GETSTRUCT(existing[nbr_index]);
		...
		else
		{
			...
			other_nbr_en = (Form_kmd_enum) GETSTRUCT(existing[other_nbr_index]);
			...
			{
				RenumberEnumType(kmd_enum, existing, nelems);
	...
	if (IsBinaryUpgrade)
	{
		if (!OidIsValid(binary_upgrade_next_kmd_enum_oid))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("kmd_enum OID value not set when in binary upgrade mode")));

		/*
		 * Use binary-upgrade override for kmd_enum.oid, if supplied. During
		 * binary upgrade, all kmd_enum.oid's are set this way so they are
		...
		newOid = binary_upgrade_next_kmd_enum_oid;
		binary_upgrade_next_kmd_enum_oid = InvalidOid;
	}
	else
	{
		/*
		...
		for (;;)
		{
			...
			/* Get a new OID (different from all existing kmd_enum tuples) */
			newOid = GetNewOidWithIndex(kmd_enum, EnumOidIndexId,
										Anum_kmd_enum_oid);
			...
			for (i = 0; i < nelems; i++)
			{
				...
				Form_kmd_enum exists_en = (Form_kmd_enum) GETSTRUCT(exists_tup);
	...
	/* Create the new kmd_enum entry */
	memset(nulls, false, sizeof(nulls));
	values[Anum_kmd_enum_oid - 1] = ObjectIdGetDatum(newOid);
	values[Anum_kmd_enum_enumtypid - 1] = ObjectIdGetDatum(enumTypeOid);
	values[Anum_kmd_enum_enumsortorder - 1] = Float4GetDatum(newelemorder);
	namestrcpy(&enumlabel, newVal);
	values[Anum_kmd_enum_enumlabel - 1] = NameGetDatum(&enumlabel);
	enum_tup = heap_form_tuple(RelationGetDescr(kmd_enum), values, nulls);
	CatalogTupleInsert(kmd_enum, enum_tup);
				...
				const char *newVal)
{
	 ...
	 * use.  (The unique index on kmd_enum would catch that anyway, but we
	...
	/* Update the kmd_enum entry */
	namestrcpy(&en->enumlabel, newVal);
	CatalogTupleUpdate(kmd_enum, &enum_tup->t_self, enum_tup);
	heap_freetuple(enum_tup);
 ...
 * it will never happen.  The reason is that updating existing kmd_enum
 * entries creates hazards for other backends that are concurrently reading
 * kmd_enum.  Although system catalog scans now use MVCC semantics, the
 * syscache machinery might read different kmd_enum entries under different
 ...
 * all the relevant kmd_enum entries with a single MVCC snapshot, or else
 ...
 */
static void
RenumberEnumType(Relation kmd_enum, HeapTuple *existing, int nelems)
{
	int			i;
	...
	for (i = nelems - 1; i >= 0; i--)
	{
		...
		en = (Form_kmd_enum) GETSTRUCT(newtup);
		...
		if (en->enumsortorder != newsortorder)
		{
			...
			CatalogTupleUpdate(kmd_enum, &newtup->t_self, newtup);
		}
...
sort_order_cmp(const void *p1, const void *p2)
{
	...
	Form_kmd_enum en1 = (Form_kmd_enum) GETSTRUCT(v1);
	Form_kmd_enum en2 = (Form_kmd_enum) GETSTRUCT(v2);

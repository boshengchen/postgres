 ...
 *
 * kmd_inherits.c
 *	  routines to support manipulation of the kmd_inherits relation
 *
 * Note: currently, this module only contains inquiry functions; the actual
 * creation and deletion of kmd_inherits entries is done in tablecmds.c.
 * Perhaps someday that code should be moved here, but it'd have to be
 * disentangled from other stuff such as kmd_depend updates.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/kmd_inherits.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
find_inheritance_children(Oid parentrelId, LOCKMODE lockmode)
{
	...
	/*
	 * Can skip the scan if kmd_class shows the relation has never had a
	 * subclass.
	 */
	...
	/*
	 * Scan kmd_inherits and build a working array of subclass OIDs.
	 */
	maxoids = 32;
	...
	{
		inhrelid = ((Form_kmd_inherits) GETSTRUCT(inheritsTuple))->inhrelid;
		if (numoids >= maxoids)
		{
			maxoids *= 2;
 ...
 * This is because relhassubclass in kmd_class is not updated immediately
 ...
 *
 * Although this doesn't actually touch kmd_inherits, it seems reasonable
...
has_subclass(Oid relationId)
{
	...
	result = ((Form_kmd_class) GETSTRUCT(tuple))->relhassubclass;
...
has_superclass(Oid relationId)
{
	...
	ScanKeyInit(&skey, Anum_kmd_inherits_inhrelid, BTEqualStrategyNumber,
...
typeInheritsFrom(Oid subclassTypeId, Oid superclassTypeId)
{
	...
	foreach(queue_item, queue)
	{
		 ...
		 * infinite loop in case there is a cycle in kmd_inherits (though
		...
		{
			Form_kmd_inherits inh = (Form_kmd_inherits) GETSTRUCT(inhtup);
...
StoreSingleInheritance(Oid relationId, Oid parentOid, int32 seqNumber)
{
	...
	/*
	 * Make the kmd_inherits entry
	 */
	values[Anum_kmd_inherits_inhrelid - 1] = ObjectIdGetDatum(relationId);
	values[Anum_kmd_inherits_inhparent - 1] = ObjectIdGetDatum(parentOid);
	values[Anum_kmd_inherits_inhseqno - 1] = Int32GetDatum(seqNumber);
 ...
 * DeleteInheritsTuple
 *
 * Delete kmd_inherits tuples with the given inhrelid.  inhparent may be given
...
DeleteInheritsTuple(Oid inhrelid, Oid inhparent)
{
	...
	/*
	 * Find kmd_inherits entries by inhrelid.
	 */
	...
	while (HeapTupleIsValid(inheritsTuple = systable_getnext(scan)))
	{
		...
		parent = ((Form_kmd_inherits) GETSTRUCT(inheritsTuple))->inhparent;

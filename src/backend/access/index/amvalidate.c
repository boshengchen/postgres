...
identify_opfamily_groups(CatCList *oprlist, CatCList *proclist)
{
	...
	Form_kmd_amop oprform;
	Form_kmd_amproc procform;
	int			io,
				ip;
	...
	{
		oprform = (Form_kmd_amop) GETSTRUCT(&oprlist->members[io]->tuple);
		io++;
	}
	else
		oprform = NULL;
	...
	{
		procform = (Form_kmd_amproc) GETSTRUCT(&proclist->members[ip]->tuple);
		ip++;
	}
	else
		procform = NULL;
 ...
 * come from kmd_amop and should always match the operator exactly.
 */
bool
check_amop_signature(Oid opno, Oid restype, Oid lefttype, Oid righttype)
{
	...
	opform = (Form_kmd_operator) GETSTRUCT(tp);
...
opfamily_can_sort_type(Oid opfamilyoid, Oid datatypeoid)
{
	...
	for (i = 0; i < opclist->n_members; i++)
	{
		...
		Form_kmd_opclass classform = (Form_kmd_opclass) GETSTRUCT(classtup);

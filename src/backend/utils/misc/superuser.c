 ...
 * kmd_authid.rolsuper directly, so that the escape hatch built in for
 ...
 * at need by watching for cache update events on kmd_authid.
 */
...
superuser_arg(Oid roleid)
{
	...
	/* OK, look up the information in kmd_authid */
	rtup = SearchSysCache1(AUTHOID, ObjectIdGetDatum(roleid));
	if (HeapTupleIsValid(rtup))
	{
		result = ((Form_kmd_authid) GETSTRUCT(rtup))->rolsuper;

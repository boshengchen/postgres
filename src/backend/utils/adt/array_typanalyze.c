 ...
 * too much space in the resulting kmd_statistic rows, we ignore arrays that
 ...
 * array.  Therefore, we store in the finished kmd_statistic entry each
					...
					int samplerows, double totalrows)
{
	 ...
	 * the number of individual elements tracked in kmd_statistic ought to be
	...
	/* Skip kmd_statistic slots occupied by standard statistics */
	slot_idx = 0;
	while (slot_idx < STATISTIC_NUM_SLOTS && stats->stakind[slot_idx] != 0)
		slot_idx++;
	if (slot_idx > STATISTIC_NUM_SLOTS - 2)
		elog(ERROR, "insufficient kmd_statistic slots for array stats");

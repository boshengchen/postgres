 ...
 *		Flush all cache entries when kmd_tablespace is updated.
 *
 * When kmd_tablespace is updated, we must flush the cache entry at least
...
get_tablespace(Oid spcid)
{
	...
	/*
	 * Since spcid is always from a kmd_class tuple, InvalidOid implies the
	 * default.
	 */
	if (spcid == InvalidOid)
	...
	else
	{
		...
		datum = SysCacheGetAttr(TABLESPACEOID,
								tp,
								Anum_kmd_tablespace_spcoptions,
								&isNull);
		if (isNull)
			opts = NULL;
		else
	 ...
	 * reading the kmd_tablespace entry, since doing so could cause a cache
	 * flush.
	 */
	spc = (TableSpaceCacheEntry *) hash_search(TableSpaceCacheHash,
											   (void *) &spcid,

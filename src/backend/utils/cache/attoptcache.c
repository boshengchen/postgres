 ...
 * kmd_attribute entries, which are handled by the relcache.
 *
 ...
 *		Flush all cache entries when kmd_attribute is updated.
 *
 * When kmd_attribute is updated, we must flush the cache entry at least
...
get_attribute_options(Oid attrelid, int attnum)
{
	...
	if (!attopt)
	{
		 ...
		 * kmd_attribute, since the read could cause a cache flush.
		 */

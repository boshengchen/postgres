 ...
 */
static const FormData_kmd_attribute Desc_kmd_class[Natts_kmd_class] = {Schema_kmd_class};
static const FormData_kmd_attribute Desc_kmd_attribute[Natts_kmd_attribute] = {Schema_kmd_attribute};
static const FormData_kmd_attribute Desc_kmd_proc[Natts_kmd_proc] = {Schema_kmd_proc};
static const FormData_kmd_attribute Desc_kmd_type[Natts_kmd_type] = {Schema_kmd_type};
static const FormData_kmd_attribute Desc_kmd_database[Natts_kmd_database] = {Schema_kmd_database};
static const FormData_kmd_attribute Desc_kmd_authid[Natts_kmd_authid] = {Schema_kmd_authid};
static const FormData_kmd_attribute Desc_kmd_auth_members[Natts_kmd_auth_members] = {Schema_kmd_auth_members};
static const FormData_kmd_attribute Desc_kmd_index[Natts_kmd_index] = {Schema_kmd_index};
static const FormData_kmd_attribute Desc_kmd_shseclabel[Natts_kmd_shseclabel] = {Schema_kmd_shseclabel};
static const FormData_kmd_attribute Desc_kmd_subscription[Natts_kmd_subscription] = {Schema_kmd_subscription};
...
typedef struct opclasscacheent
{
	...
	StrategyNumber numSupport;	/* max # of support procs (from kmd_am) */
...
static void formrdesc(const char *relationName, Oid relationReltype,
					  bool isshared, int natts, const FormData_kmd_attribute *attrs);

static HeapTuple ScanPgRelation(Oid targetRelId, bool indexOK, bool force_non_historic);
static Relation AllocateRelationDesc(Form_kmd_class relp);
 ...
 *		ScanPgRelation
 *
 *		This is used by RelationBuildDesc to find a kmd_class
 ...
 *		an attribute were to be added after scanning kmd_class and before
 *		scanning kmd_attribute, relnatts wouldn't match.
 *
...
ScanPgRelation(Oid targetRelId, bool indexOK, bool force_non_historic)
{
	...
	SysScanDesc kmd_class_scan;
	ScanKeyData key[1];
	Snapshot	snapshot;

	/*
	 * If something goes wrong during backend startup, we might find ourselves
	 * trying to read kmd_class before we've selected a database.  That ain't
	 * gonna work, so bail out with a useful error message.  If this happens,
	 * it probably means a relcache entry that needs to be nailed isn't.
	 */
	if (!OidIsValid(MyDatabaseId))
		elog(FATAL, "cannot read kmd_class without having selected a database");
	...
	/*
	 * Open kmd_class and fetch a tuple.  Force heap scan if we haven't yet
	 * built the critical relcache entries (this includes initdb and startup
	 * without a pg_internal.init file).  The caller can also force a heap
	 * scan by setting indexOK == false.
	 */
	kmd_class_desc = table_open(RelationRelationId, AccessShareLock);
	...
	kmd_class_scan = systable_beginscan(kmd_class_desc, ClassOidIndexId,
									   indexOK && criticalRelcachesBuilt,
									   snapshot,
									   1, key);

	kmd_class_tuple = systable_getnext(kmd_class_scan);

	/*
	 * Must copy tuple before releasing buffer.
	 */
	if (HeapTupleIsValid(kmd_class_tuple))
		kmd_class_tuple = heap_copytuple(kmd_class_tuple);

	/* all done */
	systable_endscan(kmd_class_scan);
	table_close(kmd_class_desc, AccessShareLock);

	return kmd_class_tuple;
}
 ...
 *		and initialize the rd_rel field from the given kmd_class tuple.
 */
static Relation
AllocateRelationDesc(Form_kmd_class relp)
{
	Relation	relation;
	MemoryContext oldcxt;
	Form_kmd_class relationForm;
	 ...
	 * in rd_options).
	 */
	relationForm = (Form_kmd_class) palloc(CLASS_TUPLE_SIZE);
 ...
 *		Convert kmd_class.reloptions into pre-parsed rd_options
 *
 * tuple is the real kmd_class tuple (not rd_rel!) for relation
 *
...
RelationParseRelOptions(Relation relation, HeapTuple tuple)
{
	 ...
	 * we might not have any other for kmd_class yet (consider executing this
	 * code for kmd_class itself)
	 */
 ...
 *		the kmd_attribute, kmd_attrdef & kmd_constraint system catalogs.
 */
static void
RelationBuildTupleDesc(Relation relation)
{
	HeapTuple	kmd_attribute_tuple;
	Relation	kmd_attribute_desc;
	SysScanDesc kmd_attribute_scan;
	ScanKeyData skey[2];
	int			need;
	TupleConstr *constr;
	...
	/* copy some fields from kmd_class row to rd_att */
	...
	/*
	 * Open kmd_attribute and begin a scan.  Force heap scan if we haven't yet
	 * built the critical relcache entries (this includes initdb and startup
	 * without a pg_internal.init file).
	 */
	kmd_attribute_desc = table_open(AttributeRelationId, AccessShareLock);
	kmd_attribute_scan = systable_beginscan(kmd_attribute_desc,
	...
	while (HeapTupleIsValid(kmd_attribute_tuple = systable_getnext(kmd_attribute_scan)))
	{
		Form_kmd_attribute attp;
		int			attnum;

		attp = (Form_kmd_attribute) GETSTRUCT(kmd_attribute_tuple);
		...
		if (attp->atthasmissing)
		{
			...
			missingval = heap_getattr(kmd_attribute_tuple,
									  Anum_kmd_attribute_attmissingval,
									  kmd_attribute_desc->rd_att,
									  &missingNull);
			if (!missingNull)
			{
	 ...
	 */
	systable_endscan(kmd_attribute_scan);
	table_close(kmd_attribute_desc, AccessShareLock);
	...
	/*
	 * The attcacheoff values we read from kmd_attribute should all be -1
 ...
 *		the kmd_rewrite system catalog.
 *
...
RelationBuildRuleLock(Relation relation)
{
	...
	/*
	 * open kmd_rewrite and begin a scan
	 *
	...
	{
		Form_kmd_rewrite rewrite_form = (Form_kmd_rewrite) GETSTRUCT(rewrite_tuple);
		bool		isnull;
		Datum		rule_datum;
		char	   *rule_str;
		RewriteRule *rule;
 ...
 *
 *		Returns NULL if no kmd_class row could be found for the given relid
...
RelationBuildDesc(Oid targetRelId, bool insertIt)
{
	...
	/*
	 * find the tuple in kmd_class corresponding to the given relation id
	 */
	kmd_class_tuple = ScanPgRelation(targetRelId, true, false);
	...
	/*
	 * get information from the kmd_class_tuple
	 */
	relp = (Form_kmd_class) GETSTRUCT(kmd_class_tuple);
	relid = relp->oid;
	Assert(relid == targetRelId);

	/*
	 * allocate storage for the relation descriptor, and copy kmd_class_tuple
	 * to relation->rd_rel.
	 */
	...
	switch (relation->rd_rel->relpersistence)
	{
		...
		case RELPERSISTENCE_TEMP:
			...
			else
			{
				 ...
				 * here, in case we are looking at a kmd_class entry left over
				 ...
				 * rd_islocaltemp flag.  The kmd_class entry will get flushed
	...
	RelationParseRelOptions(relation, kmd_class_tuple);
	...
	/*
	 * now we can free the memory allocated for kmd_class_tuple
	 */
	...
	if (relation->rd_rel->relfilenode)
	{
			...
			&& IsTransactionState())
		{
			...
			if (!HeapTupleIsValid(phys_tuple))
				elog(ERROR, "could not find kmd_class entry for %u",
					 RelationGetRelid(relation));
			physrel = (Form_kmd_class) GETSTRUCT(phys_tuple);
...
RelationInitIndexAccessInfo(Relation relation)
{
	...
	/*
	 * Make a copy of the kmd_index entry for the index.  Since kmd_index
	 * contains variable-length and possibly-null fields, we have to do this
	 * honestly rather than just treating it as a Form_kmd_index struct.
	 */
	...
	relation->rd_index = (Form_kmd_index) GETSTRUCT(relation->rd_indextuple);
	...
	aform = (Form_kmd_am) GETSTRUCT(tuple);
 ...
 *		given the index's kmd_index.indclass entry.
 *
					   ...
					   AttrNumber maxAttributeNumber)
{
	...
	for (attIndex = 0; attIndex < maxAttributeNumber; attIndex++)
	{
		...
		if (!OidIsValid(indclass->values[attIndex]))
			elog(ERROR, "bogus kmd_index tuple");
 ...
 * opclass in a single indexscan of kmd_amproc.
 *
 * The information from kmd_am about expected range of support function
				  ...
				  StrategyNumber numSupport)
{
	...
	/*
	 * We have to fetch the kmd_opclass row to determine its opfamily and
	...
	{
		Form_kmd_opclass opclassform = (Form_kmd_opclass) GETSTRUCT(htup);
	...
	/*
	 * Scan kmd_amproc to obtain support procs for the opclass.  We only fetch
	...
	if (numSupport > 0)
	{
		...
		{
			Form_kmd_amproc amprocform = (Form_kmd_amproc) GETSTRUCT(htup);
...
formrdesc(const char *relationName, Oid relationReltype,
		  bool isshared,
		  int natts, const FormData_kmd_attribute *attrs)
{
	Relation	relation;
	int			i;
	bool		has_not_null;
	 ...
	 * data from kmd_class and replace what we've done here.  Note in
	 * particular that relowner is left as zero; this cues
	 * RelationCacheInitializePhase3 that the real data isn't there yet.
	 */
	relation->rd_rel = (Form_kmd_class) palloc0(CLASS_TUPLE_SIZE);
...
RelationIdGetRelation(Oid relationId)
{
	...
	if (RelationIsValid(rd))
	{
		...
		if (!rd->rd_isvalid)
		{
			 ...
			 * perform kmd_class lookups. The structure of such entries doesn't
 ...
 *	can mean that its kmd_class or kmd_index row changed.  There are only
 ...
 *	and/or in active use.  We support full replacement of the kmd_class row,
 *	as well as updates of a few simple fields of the kmd_index row.
 *
 ...
 *
 *	If the target index is an index on kmd_class or kmd_index, we'd better have
...
RelationReloadIndexInfo(Relation relation)
{
	 ...
	 * kmd_class yet.  However, a shared index can never have any significant
	...
	/*
	 * Read the kmd_class row
	 *
	 * Don't try to use an indexscan of kmd_class_oid_index to reload the info
	 * for kmd_class_oid_index ...
	 */
	indexOK = (RelationGetRelid(relation) != ClassOidIndexId);
	kmd_class_tuple = ScanPgRelation(RelationGetRelid(relation), indexOK, false);
	if (!HeapTupleIsValid(kmd_class_tuple))
		elog(ERROR, "could not find kmd_class tuple for index %u",
			 RelationGetRelid(relation));
	relp = (Form_kmd_class) GETSTRUCT(kmd_class_tuple);
	...
	RelationParseRelOptions(relation, kmd_class_tuple);
	/* done with kmd_class tuple */
	heap_freetuple(kmd_class_tuple);
	/* We must recalculate physical address in case it changed */
	RelationInitPhysicalAddr(relation);

	/*
	 * For a non-system index, there are fields of the kmd_index row that are
	 * allowed to change, so re-read that row and update the relcache entry.
	 * Most of the info derived from kmd_index (such as support function lookup
 ...
 * some parts, e.g. kmd_class.relfrozenxid, are still important to have
...
RelationReloadNailed(Relation relation)
{
	...
	if (relation->rd_rel->relkind == RELKIND_INDEX)
	{
		 ...
		 * kmd_class row to see if its relfilenode changed.
		 */
	...
	else
	{
		...
		if (criticalRelcachesBuilt)
		{
			 ...
			 * self-recursion when re-building kmd_class.
			 */
			relation->rd_isvalid = true;

			kmd_class_tuple = ScanPgRelation(RelationGetRelid(relation),
											true, false);
			relp = (Form_kmd_class) GETSTRUCT(kmd_class_tuple);
			memcpy(relation->rd_rel, relp, CLASS_TUPLE_SIZE);
			heap_freetuple(kmd_class_tuple);
...
RelationClearRelation(Relation relation, bool rebuild)
{
	 ...
	 * re-read the kmd_class row to handle possible physical relocation of the
	 * index, and we check for kmd_index updates too.
	 */
	if ((relation->rd_rel->relkind == RELKIND_INDEX ||
		 relation->rd_rel->relkind == RELKIND_PARTITIONED_INDEX) &&
		relation->rd_refcnt > 0 &&
	...
	else
	{
		 ...
		 * attempt to preserve the kmd_class entry (rd_rel), tupledesc,
...
#define SWAPFIELD(fldtype, fldname) \
		...
		SWAPFIELD(Form_kmd_class, rd_rel);
...
RelationCacheInvalidate(void)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		else
		{
			 ...
			 * kmd_class goes to the front of rebuildFirstList while
			 * kmd_class_oid_index goes to the back of rebuildFirstList, so
						   ...
						   char relkind)
{
	...
	for (i = 0; i < natts; i++)
	{
		Form_kmd_attribute satt = TupleDescAttr(tupDesc, i);
		Form_kmd_attribute datt = TupleDescAttr(rel->rd_att, i);
	 ...
	 */
	rel->rd_rel = (Form_kmd_class) palloc0(CLASS_TUPLE_SIZE);
...
RelationSetNewRelfilenode(Relation relation, char persistence)
{
	...
	/*
	 * Get a writable copy of the kmd_class tuple for the given relation.
	 */
	...
	/*
	 * If we're dealing with a mapped index, kmd_class.relfilenode doesn't
	 * change; instead we have to send the update to the relation mapper.
	 *
	 * For mapped indexes, we don't actually change the kmd_class entry at all;
	 * this is essential when reindexing kmd_class itself.  That leaves us with
	...
	if (RelationIsMapped(relation))
	{
		...
		/* Since we're not updating kmd_class, these had better not change */
		...
		/* Since we're not updating kmd_class, must trigger inval manually */
		CacheInvalidateRelcache(relation);
	}
	else
	{
		/* Normal case, update the kmd_class entry */
	...
	/*
	 * Make the kmd_class row change or relation map change visible.  This will
 ...
 *		We must at least set up nailed reldescs for kmd_database, kmd_authid,
 *		kmd_auth_members, and kmd_shseclabel. Ideally we'd like to have reldescs
...
RelationCacheInitializePhase2(void)
{
	...
	{
		formrdesc("kmd_database", DatabaseRelation_Rowtype_Id, true,
				  Natts_kmd_database, Desc_kmd_database);
		formrdesc("kmd_authid", AuthIdRelation_Rowtype_Id, true,
				  Natts_kmd_authid, Desc_kmd_authid);
		formrdesc("kmd_auth_members", AuthMemRelation_Rowtype_Id, true,
				  Natts_kmd_auth_members, Desc_kmd_auth_members);
		formrdesc("kmd_shseclabel", SharedSecLabelRelation_Rowtype_Id, true,
				  Natts_kmd_shseclabel, Desc_kmd_shseclabel);
		formrdesc("kmd_subscription", SubscriptionRelation_Rowtype_Id, true,
				  Natts_kmd_subscription, Desc_kmd_subscription);
...
RelationCacheInitializePhase3(void)
{
		...
		!load_relcache_init_file(false))
	{
		...
		formrdesc("kmd_class", RelationRelation_Rowtype_Id, false,
				  Natts_kmd_class, Desc_kmd_class);
		formrdesc("kmd_attribute", AttributeRelation_Rowtype_Id, false,
				  Natts_kmd_attribute, Desc_kmd_attribute);
		formrdesc("kmd_proc", ProcedureRelation_Rowtype_Id, false,
				  Natts_kmd_proc, Desc_kmd_proc);
		formrdesc("kmd_type", TypeRelation_Rowtype_Id, false,
				  Natts_kmd_type, Desc_kmd_type);
...
#define NUM_CRITICAL_LOCAL_INDEXES	7	/* fix if you change list above */
	 ...
	 * need to nail up some indexes on kmd_authid and kmd_auth_members for use
...
#define NUM_CRITICAL_SHARED_INDEXES 6	/* fix if you change list above */
	 ...
	 * faked up relcache entries using formrdesc, then read the real kmd_class
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		/*
		 * If it's a faked-up entry, read the real kmd_class tuple.
		 */
		if (relation->rd_rel->relowner == InvalidOid)
		{
			...
			if (!HeapTupleIsValid(htup))
				elog(FATAL, "cache lookup failed for relation %u",
					 RelationGetRelid(relation));
			relp = (Form_kmd_class) GETSTRUCT(htup);
			...
			if (relation->rd_rel->relowner == InvalidOid)
				elog(ERROR, "invalid relowner in kmd_class entry for \"%s\"",
					 RelationGetRelationName(relation));
		 ...
		 * here.  We don't make any attempt to fix the kmd_class entry, though.
		 */
		 ...
		 * if there is no policy defined in kmd_policy.
		 */
...
/*
 * GetPgClassDescriptor -- get a predefined tuple descriptor for kmd_class
 * GetPgIndexDescriptor -- get a predefined tuple descriptor for kmd_index
 *
 * We need this kluge because we have to be able to access non-fixed-width
 * fields of kmd_class and kmd_index before we have the standard catalog caches
 ...
 */
static TupleDesc
BuildHardcodedDescriptor(int natts, const FormData_kmd_attribute *attrs)
{
	TupleDesc	result;
	MemoryContext oldcxt;
	int			i;
...
GetPgClassDescriptor(void)
{
	...
	/* Already done? */
	if (pgclassdesc == NULL)
		pgclassdesc = BuildHardcodedDescriptor(Natts_kmd_class,
											   Desc_kmd_class);
...
GetPgIndexDescriptor(void)
{
	...
	/* Already done? */
	if (pgindexdesc == NULL)
		pgindexdesc = BuildHardcodedDescriptor(Natts_kmd_index,
											   Desc_kmd_index);
...
AttrDefaultFetch(Relation relation)
{
	...
	{
		Form_kmd_attrdef adform = (Form_kmd_attrdef) GETSTRUCT(htup);
		Form_kmd_attribute attr = TupleDescAttr(relation->rd_att, adform->adnum - 1);
...
CheckConstraintFetch(Relation relation)
{
	...
	{
		Form_kmd_constraint conform = (Form_kmd_constraint) GETSTRUCT(htup);
		Datum		val;
		bool		isnull;
		char	   *s;
 ...
 * kmd_constraint.  The list items are in no particular order.
 *
...
RelationGetFKeyList(Relation relation)
{
	...
	/* Prepare to scan kmd_constraint for entries having conrelid = this rel. */
	ScanKeyInit(&skey,
				Anum_kmd_constraint_conrelid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(RelationGetRelid(relation)));
	...
	while (HeapTupleIsValid(htup = systable_getnext(conscan)))
	{
		Form_kmd_constraint constraint = (Form_kmd_constraint) GETSTRUCT(htup);
 ...
 *
 * The index list is created only if someone requests it.  We scan kmd_index
 ...
 * which is the kmd_class OID of an index to be used as the relation's
...
RelationGetIndexList(Relation relation)
{
	...
	{
		Form_kmd_index index = (Form_kmd_index) GETSTRUCT(htup);
		...
		if (!index->indisvalid || !index->indisunique ||
			!index->indimmediate ||
			!heap_attisnull(htup, Anum_kmd_index_indpred, NULL))
			continue;
 ...
 * similar to RelationGetIndexList().  We scan kmd_statistic_ext to find
...
RelationGetStatExtList(Relation relation)
{
	...
	while (HeapTupleIsValid(htup = systable_getnext(indscan)))
	{
		Oid			oid = ((Form_kmd_statistic_ext) GETSTRUCT(htup))->oid;
 ...
 *
 * We cache the result of transforming kmd_index.indexprs into a node tree.
...
RelationGetIndexExpressions(Relation relation)
{
	...
	if (relation->rd_indextuple == NULL ||
		heap_attisnull(relation->rd_indextuple, Anum_kmd_index_indexprs, NULL))
		return NIL;
 ...
 *
 * We cache the result of transforming kmd_index.indpred into an implicit-AND
...
RelationGetIndexPredicate(Relation relation)
{
	...
	/* Quick exit if there is nothing to do. */
	if (relation->rd_indextuple == NULL ||
		heap_attisnull(relation->rd_indextuple, Anum_kmd_index_indpred, NULL))
		return NIL;
...
restart:
	...
	foreach(l, indexoidlist)
	{
		...
		datum = heap_getattr(indexDesc->rd_indextuple, Anum_kmd_index_indexprs,
		...
		datum = heap_getattr(indexDesc->rd_indextuple, Anum_kmd_index_indpred,
							 GetPgIndexDescriptor(), &isnull);
		if (!isnull)
						 ...
						 uint16 **strategies)
{
	...
	*strategies = strats = (uint16 *) palloc(sizeof(uint16) * indnkeyatts);
	...
	/*
	 * Search kmd_constraint for the constraint associated with the index. To
	 * make this not too painfully slow, we use the index on conrelid; that
	 * will hold the parent relation's OID not the index's own OID.
	 *
	 * Note: if we wanted to rely on the constraint name matching the index's
	 * name, we could just do a direct lookup using kmd_constraint's unique
 ...
 *		use an index on kmd_attribute, for example, but in order to do so,
 *		we must have read kmd_attribute for the attributes in the index,
...
load_relcache_init_file(bool shared)
{
	...
	for (relno = 0;; relno++)
	{
		...
		relform = (Form_kmd_class) palloc(len);
		...
		{
			Form_kmd_attribute attr = TupleDescAttr(rel->rd_att, i);
		...
		if (rel->rd_rel->relkind == RELKIND_INDEX)
		{
			...
			rel->rd_index = (Form_kmd_index) GETSTRUCT(rel->rd_indextuple);
...
write_relcache_init_file(bool shared)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		Form_kmd_class relform = rel->rd_rel;

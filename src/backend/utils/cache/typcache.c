 ...
 * types that is not directly available from a type's kmd_type row.  For
 ...
 * detect updates to kmd_opclass.  We also support clearing the tuple
...
lookup_type_cache(Oid type_id, int flags)
{
	...
	if (typentry == NULL)
	{
		 ...
		 * kmd_type row, just to make sure we don't make a cache entry for an
...
load_rangetype_info(TypeCacheEntry *typentry)
{
	...
	/* get information from kmd_range */
	...
	if (!HeapTupleIsValid(tup))
		elog(ERROR, "cache lookup failed for range type %u",
			 typentry->type_id);
	kmd_range = (Form_kmd_range) GETSTRUCT(tup);

	subtypeOid = kmd_range->rngsubtype;
	typentry->rng_collation = kmd_range->rngcollation;
	opclassOid = kmd_range->rngsubopc;
	canonicalOid = kmd_range->rngcanonical;
	subdiffOid = kmd_range->rngsubdiff;
 ...
 * okay to leak data into the current context while scanning kmd_constraint.
...
load_domaintype_info(TypeCacheEntry *typentry)
{
	...
	/*
	 * Scan kmd_constraint for relevant constraints.  We want to find
	...
	for (;;)
	{
		...
		{
			Form_kmd_constraint c = (Form_kmd_constraint) GETSTRUCT(conTup);
			Datum		val;
			bool		isNull;
			char	   *constring;
			Expr	   *check_expr;
			...
			val = fastgetattr(conTup, Anum_kmd_constraint_conbin,
 ...
 * were a syscache-flush callback on kmd_type, but then we would need all
 * ALTER-TABLE-like commands that could modify a rowtype to issue syscache
 * invals against the rel's kmd_type OID.  The extra SI signaling could very
 ...
 *
 * This is called when a syscache invalidation event occurs for any kmd_opclass
 * row.  In principle we could probably just invalidate data dependent on the
 * particular opclass, but since updates on kmd_opclass are rare in production
 * it doesn't seem worth a lot of complication: we just mark all cached data
 * invalid.
 *
 * Note that we don't bother watching for updates on kmd_amop or kmd_amproc.
 ...
 * kmd_constraint or kmd_type row.  We flush information about domain
...
load_enum_cache_data(TypeCacheEntry *tcache)
{
	...
	/* Scan kmd_enum for the members of the target enum type. */
	ScanKeyInit(&skey,
	...
	{
		Form_kmd_enum en = (Form_kmd_enum) GETSTRUCT(enum_tuple);

						 ...
						 TupleDesc *resultTupleDesc)
{
	...
	/* First fetch the function's kmd_proc row to inspect its rettype */
	tp = SearchSysCache1(PROCOID, ObjectIdGetDatum(funcid));
	if (!HeapTupleIsValid(tp))
		elog(ERROR, "cache lookup failed for function %u", funcid);
	procform = (Form_kmd_proc) GETSTRUCT(tp);
 ...
 * kmd_proc tuple.  Return value is the total number of arguments.
 ...
 *
 * Note that this function simply fetches what is in the kmd_proc tuple;
...
{
	Form_kmd_proc procStruct = (Form_kmd_proc) GETSTRUCT(procTup);
				  ...
				  Oid **p_trftypes)
{
	...
	if (!isNull)
	{
		...
		Assert(nelems >= ((Form_kmd_proc) GETSTRUCT(procTup))->pronargs);
...
get_func_result_name(Oid functionId)
{
	...
	/* First fetch the function's kmd_proc row */
	procTuple = SearchSysCache1(PROCOID, ObjectIdGetDatum(functionId));
	if (!HeapTupleIsValid(procTuple))
		elog(ERROR, "cache lookup failed for function %u", functionId);

	/* If there are no named OUT parameters, return NULL */
	if (heap_attisnull(procTuple, Anum_kmd_proc_proargmodes, NULL) ||
		heap_attisnull(procTuple, Anum_kmd_proc_proargnames, NULL))
		result = NULL;
	else
	{
 ...
 *
 * Given a kmd_proc row for a function, return a tuple descriptor for the
...
{
	Form_kmd_proc procform = (Form_kmd_proc) GETSTRUCT(procTuple);
	Datum		proallargtypes;
	Datum		proargmodes;
	Datum		proargnames;
	bool		isnull;
	...
	if (heap_attisnull(procTuple, Anum_kmd_proc_proallargtypes, NULL) ||
		heap_attisnull(procTuple, Anum_kmd_proc_proargmodes, NULL))
		return NULL;
 ...
 *
 * Build a RECORD function's tupledesc from the kmd_proc proallargtypes,

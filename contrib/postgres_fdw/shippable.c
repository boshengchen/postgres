...
typedef struct
{
	...
	Oid			classid;		/* OID of its catalog (kmd_proc, etc) */
...
/*
 * Flush cache entries when kmd_foreign_server is updated.
 *
...
InvalidateShippableCacheCallback(Datum arg, int cacheid, uint32 hashvalue)
{
	 ...
	 * kmd_foreign_server entry being outdated; but that would be more
...
InitializeShippableCache(void)
{
	...
	/* Set up invalidation callback on kmd_foreign_server. */
